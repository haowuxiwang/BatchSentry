...
#     全链 1889s（32.7 s/页）
#   ① 云厂商的 1,000~3,000 页**不可移植** —— 页数上限是"单页成本"的函数，不是
#      可移植的常量。有出处的对照（2026-08 公开牌价，见 docs/UPLOAD_LIMITS.md §2）：
#      三大云纯文本抽取统一约 $1.50/1,000 页（=$0.0015/页）；结构化/生成式抽取
#      $10~50/1,000 页，两者相差 10~30 倍。本链路每页要跑手写体 OCR + 一次逐页
#      LLM 生成，**按调用性质属后者（最贵档）**，故同页数的代价远高于云厂商的
#      纯文本档。**可移植的量是"墙钟时间预算"，不是页数。**

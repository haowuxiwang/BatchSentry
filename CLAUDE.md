...
**Current phase**: **v1.1.9**（可分发版本；Round 41 **基线冻结 + 全历史密钥卫生实测复核**（3 把历史 `sk-` 全部 401 invalid ⇒ 泄露面关闭）、Round 42 **产物级端到端测试**（用 `tests/e2e_run.py` 驱动 **Electron 内嵌的那份 exe**，**7 轮全过**，含 **51 页真实批记录** 293 findings；并用**视觉直读原页**核实出 3 类 **LLM 层**假阳性）、Round 43 **第三轮对抗性审查**（`docs/ADVERSARIAL_AUDIT.md` **§15**：五路只读审查 + 视觉直读原页交叉比对 + 产物实测；**判定无 P0、v1.1.9 可作基线**；新增 P1×1 / P2×7 / P3×9，其中 P1 = provider 被启动逻辑自动改写并持久化。**本轮只动文档与清单，不升版**）。Round 44 **B1-6「收权」**（新增 `core/rules/llm_finding_guard.py`：LLM findings 落库前过**四层确定性复核** —— L1 值形态 / L2 溯源 / L3 判据重算 / L4 severity 封顶；接入 `stage2`（llm_page）与 `stage3`（llm_cross/llm_fallback）两条路径，抑制**必留痕**进 `finding_suppressions` 台账；51 页真实回放 **critical 49 → 0**、rule 层真阳性零误杀）。Round 45 **B1-9**（`_check_time_reversal_cross_page` 判据由「区间重叠」纠正为「真倒序」，并按证据强度封顶：手写填写 / 时刻字面量缺日期成分 ⇒ **一律不给 critical**）。Round 46 **第四轮对抗性审查**（**审查对象是尚未分发的 B1-6/B1-9**；抓出**同一个工程错误在两条 L3 路径上各犯一次** —— 把「**判不了**」当成「**判据确凿**」而误抑制：p38 的 **2027 未来日期**、p43×4 / p46 文案**明写**「开始时间晚于结束时间」的真倒序均被冤枉抑制，且**无兜底**；当日修复为「**弱证据只降级、强证据才抑制**」—— 抑制 **26 → 13**、降级 42 → 55、**critical 保持 0**，护栏 20 → 23 例 + 变异 3 方向全红；新登记 B1-12 / B2-12 / B3-4；**判定本轮不冻结 v1.2.0**：`app.asar` 被宿主 `WorkBuddy` 持有 ⇒ 构建会自愈到时间戳目录，违反产物目录唯一性）。Round 47 **B3-4**（用户指定处理：`apply_review` 是死代码、且 stage2/stage3 各自手抄同一段「重建 + 降级物化」逻辑并**已漂移** ⇒ ① 两处调用方改为统一调用 `apply_review`，**写路径只剩一个实现点**、漂移随之消除；② 借一次**全仓死接口扫描**（`devlogs/_lint/deadscan.py`，判据 = 生产侧零引用）把同类一次性清零 **7 → 0** —— 其中 3 个"函数是死的、但生产侧**内联了同一逻辑**"按同一治法**统一**（`source_version` / `entries_by_source` / `is_canonical`），3 个纯负债**删除**（`rule_by_type` 与 `rule_coverage` 的 `by_type` 语义不同故只能删、`begin_children` 是冗余第二构造入口、`vote_report` docstring 谎称"落 audit"，缺口另登记 **B3-5**）；③ 新增护栏 `tests/unit/test_llm_guard_single_impl.py` **6 例**（AST 精确断言 + **正向对照**防"空断言"），**变异 4 方向全部符合预期**且逐字节还原自校验通过；全量 **2776 passed / 0 failed**；**再次实测确认仍不可打包** —— 用 `clean_dist.locked_files()` 直接探针（非推断）：`app.asar` 改名报 `winerror=32`（共享冲突）⇒ 目录不可改名 ⇒ 构建必自愈到时间戳目录，故**不升版**，留 v1.2.0 统一升版重建）。**上一版 v1.1.8**（可分发版本；Round 32 修**失败页在任务列表里被静默丢弃**（#132）：`jobs.failed_pages` 是 SQLite TEXT 列（存 JSON），JSON 接口**原样透传** ⇒ 响应里是字符串 `"[2, 1]"` ⇒ 前端 `Array.isArray()` 拿到即退化成 `[]` ⇒ **失败页数与页码在任务列表里永不显示**；而复核页（SSR，自行 `json.loads`）显示正常 ⇒ 同一字段两处不一致，用户只会读成"这次没有失败页"。改动落在 `api/jobs/status.py`（新增 `_parse_failed_pages`，接入 `GET /{id}` 与 SSE 两个入口）与 `main.py`（复核页改为复用同一解析器）——**两者都进 PyInstaller 产物 ⇒ 升版 1.1.8 并重建**）。**上一版 v1.1.7**（可分发版本；Round 30 加**仓库卫生机检**（生成物**绝不允许入库** + `dist*` 变体堆积**可见化** + 「文档允许 agent 写入的落点」必须与忽略清单/门禁**三处联动**），Round 31 修 **e2e 夹具的"假绿"**与 **`clean_dist` 认锁探测的"假慢"**。核心洞察：*文档里的规则不会自我执行* —— 规则让 agent 往哪儿写，忽略清单就必须在哪儿拦，否则 agent 会**照规则做错事**。改动只落 `.gitignore`、`scripts/`、`tests/`、文档，**不进 PyInstaller 产物**（已核实产物 `_internal` 内无 `scripts/`）⇒ 按既有约定**不升版号**，记 `[Unreleased]`）— 修**同一份响应自相矛盾**（缺陷 #131）：「已分析页数」有**两套口径** —— 状态接口按 `structured_json IS NOT NULL` 计数，而失败页**同样**会写入 `structured_json`（带 `_parse_error` 标记）⇒ 终态 `error` 的原因文本写「0 页产出可用结果」，同一份 payload 里 `pages_analyzed` 却是 **1**（`failed_pages=[1,2]`）；`partial_review` 文案同样虚高（`部分可复核 · 10/10 页` 而实际只有 8 页真产出）。修法 = 接口**复用**规范函数 `_get_analyzed_pages`（**同一个函数**，而非抄同一段 SQL），两个入口（`GET /{job_id}` 与 SSE 的 `_get_job_progress`）同口径。由 #127 的**产物级实景验收**现场抓出（不是推断）。上一版 **v1.1.6** 修**「分析没跑成」与「记录没问题」在界面上长得一样**（缺陷 #127）：模型凭据失效时每页 Stage 2 都以 401 失败，job 却报 `partial_review`、`error_message` 为 NULL、0 条 finding，界面是**绿点 +「部分可复核」** —— 对 GMP 复核者这是**假阴性**。修法 = 故障分类**随异常一起上抛**（`LLMConfigError(RuntimeError)`）+ 首因**提升到 job 级** + **早停**（不再对同一个确定失败打 N 次）+ **零页产出落 `error`**（判据从数据派生）+ 前端**非绿点 / 显原因 / 显失败页**。上一版 **v1.1.5** 修**上游「容量类」错误的处置**（缺陷 #120）：旋转补救把上游拥塞当永久失败，**一个角度都没读到**（现场五次探测全被 Paddle `code:10010 任务提交队列已满` 拒），还把基础设施状况写成「已探测全部角度未果」的**内容结论**；修法 = 错误分类**单一真值** + 拥塞**退避重试同一角度**（不消耗角度预算）+ **区分性诊断**（`rotation_blocked` 与 `rotation_probed` 互斥）+ 双预算。顺带查出并修掉同源缺陷（`OCRCancelled` 被 `except Exception` 吞成"探测失败"，取消后还会再打上游）。并落地 P2「**停滞可见性**」（**纯派生**：不写库、不改 schema、不新增 SSE 事件类型）。此前 **v1.1.4** 做看门狗实施后回审（P1 自身 4 项缺陷 + `GET /api/health/watchdog` + e2e 轮次预算公式化），并回审**验证链自身**（e2e 护栏 2 处真实性缺陷）与把"旋转自愈疑似回归"定案为**上游拥塞产物**。覆盖率门禁 **95%**；真实 51 页全链路 frozen e2e 为放行前提。更早 v1.1.2 补完 P0-3 的**服务端转正页面逆映射**（第 8 页 `angle=270`）、**分发一致性机检**与**上传页数上限**。更早：v1.1 全部里程碑 M2–M7、Phase 12（Feishu job notifications）、2 轮对抗审查，v1.0.0，本地单用户部署（PyInstaller exe + Electron 便携版 win-unpacked）。
...
**Round 60 (分发就绪 W2 收尾：重建 + 新 key e2e + 门禁全绿, 2026-09-24)**: 承接三周演进（W1 守卫前移/门禁 11 项/CVE 可达性 → W2 依赖升级，详见 `CHANGELOG.md` 与 `docs/TODO.md`「分发就绪计划」D1-D6）。① **重建便携包**：Electron **43.7.4**（钉精确版——43.7.5 的 win32 二进制在四家镜像全部 404）/ Chrome 150，475.1 MB，磁盘**唯一** `dist-electron`（旧归档经 `clean_dist.py` 入回收站）。② **新 key e2e 全绿**（DeepSeek-V3.2 + 新 Paddle token）：LLM 连通性 1691ms + 读回生效值一致；pdf 轮 6 页 28 findings（gmp_basis 26/28，missing=[]）；rot 轮 288s rotation@90°×2 恢复（gmp_basis 16/16）；cancel 轮 35s 干净取消（138MB 真实 PDF，cancel 语义回归）。③ **对抗性审查修掉三处真缺陷**：**(a) PROVENANCE 契约断裂**——门禁 `count_complete_artifacts` 把 PROVENANCE.txt 列为完整产物三件套之一，而 build.ps1 只在备用目录写 ⇒ 标准路径产物**永远被判残壳**、`runtime_eol` 恒 SKIP（旧轮次未暴露只因当时完整产物恰好来自备用目录）；改**无条件**写入。修复两课入档：Edit 工具会**丢 UTF-8 BOM**（PowerShell 5.1 对无 BOM 中文脚本按 ANSI 解析 ⇒ 满屏语法错）；PowerShell 语句位**裸 `{...}` 是 scriptblock 表达式**（被输出、不执行——首版修复"看似成功实则从未写入"，日志里打印的正是 scriptblock 对象本身）。**(b) 门禁自测环境耦合**——`test_child_receives_raised_threshold` 隐含假设宿主必有 `CODEBUDDY_SAFE_DELETE_BULK_THRESHOLD`，在 TRAE/CI 恒红；改测试自注入前置 + 补反向对照。**(c)** CHANGELOG 误写 Electron 43.7.5。④ **门禁 11/11 全绿**（10 PASS + tests_coverage 3252 passed / 95.04%），`runtime_eol`/`dist_variants`/`dependency_vulns` 历史 FAIL 全转绿；提交 `430c212`。⑤ **遗留**：B11-5 精确吞吐实测（e2e 驱动无吞吐打点，工具缺口）、B11-13 ≥120 页实跑、B11-14 关闭语义收尾、B11-15 CPU 移出事件循环；GitHub push 被网络挡（代理 7897 未开 + 直连 reset，5 提交待推送）。

**Round 61 (v1.2.1 报告查看页与优雅返回, 2026-09-24)**: 用户报告主流程复核页点"下载报告"后无法优雅返回主界面。① **先定位**：按钮是裸链 `/api/jobs/{id}/report.md`（纯文本端点），浏览器直接导航后脱离站点外壳（顶栏消失），只能靠浏览器后退。② **方案**（按用户要求"不支持下下载、只支持查看"）：入口改站内 SSR 报告页 `GET /jobs/{id}/report` + 顶栏"← 返回复核 / 设置 / 首页"。路由状态机：未知 job 404 / 非终态（仍在跑/出错/取消）**303 回复核页**（没有可看的报告，回去看进度/原因）/ 终态（review / partial_review / done）渲染。md 内容与 `.md` 端点**共用同一缓存函数** `_generate_report_md_cached`（复核状态变化自动失效），不走第二套生成逻辑；`.md` API 端点保留（e2e 与下游契约）。③ **零新依赖渲染器** `core/md_render.py`：便携包离线分发，为一个页面引入 markdown 库不成比例。子集 = 报告 md 实际形态（标题/两级嵌套列表/**粗体**/`行内代码`/hr）；**安全模型** = 信任生成端 `esc()` 前置转义（cr-7/P1-B 已把 HTML 与 md 元字符转实体），渲染器只加结构性标签、实体原样透传；超子集（表格/引用块）按段落降级、**永不抛错**。一处测试初稿把 7 个 `#` 写成"钳到 h6"——实按 CommonMark 是**段落**，修的是测试不是实现。④ **测试**：单测 37 条（精确 HTML 串断言 + 实体透传 + 真实报告片段端到端；变异 CAUGHT：删粗体处理 → 5 红）；集成 19 条（路由状态机参数化 / 返回按钮 / 内容 / review 入口替换完整性 / `.md` 契约；变异 CAUGHT：拆非终态重定向 → 5 红）。⑤ **升版 1.2.0 → 1.2.1**（APP_VERSION / package.json / package-lock 双处，一致性机检 4 条过）+ CHANGELOG 段落。

**Round 33 (A1 归因更正：锁的持有者是宿主进程，不是安全软件；探针纪律再收一条, 2026-09-17)**: 用户要求"继续"，先复核两个外部阻塞项 ⇒ **A4（SiliconFlow 402 余额不足）复现不变**，而 **A1 的归因被实测推翻**。① **旧结论**：`dist-electron/.../app.asar` 无法改名/删除被记作"**安全软件（火绒）驱动级锁**"，处置写成"把仓库加入信任区/白名单"。② **本次定位**：用 **Restart Manager**（`rstrtmgr.dll` 的 `RmStartSession/RmRegisterResources/RmGetList`）直接**具名**持有者 —— **`WorkBuddy.exe`（pid 16220 / 14048，宿主自身）**；范围探测显示**只有 `*.asar` 被占**，同目录 `pbc-server.exe` / `BatchSentry.exe` / `app.asar.unpacked` 全部 FREE ⇒ 过滤驱动式"整目录拦截"可以排除。③ **可复现的控制实验**：新建 2 MB 仿 asar 头文件 → **7 次采样 / 13 秒全空闲**（⇒ 不是"见 asar 就抓"的扫描器）；**用宿主读取通道打开一次 ⇒ 立刻 `winerror=32` 且持续**；对照读 `*.txt`/`*.exe`/`*.dll` → 仍空闲（⇒ 不是"读文件就锁"，而是 **asar 专属通道**：宿主把 `.asar` 当"包"打开后留下未带 `FILE_SHARE_DELETE` 的句柄）。④ **机制澄清 = 仓库卫生问题的根因**：拿宿主读取能力去"看一眼打包版本"就会把产物目录锁死 ⇒ 下次 electron-builder 清不掉 `dist-electron` ⇒ 自愈到时间戳目录 ⇒ **目录越堆越多**。故核验 `app.asar` **一律走子进程**（`clean_dist.asar_version()` 即安全做法；实测读出 `dist-electron`=**1.1.7** / 时间戳目录=**1.1.8**，据此决定留哪一个，不靠 mtime 猜）。⑤ **修法（工具侧，不动产品代码 ⇒ 不进产物、不升版号）**：`scripts/clean_dist.py` 新增 `who_holds()` / `describe_holders()`（**fail-soft**：非 Windows / dll 缺失 / 任何异常都只返回空表，绝不让诊断把清理工具带崩），报告里**直接具名持有者**（不再用 `tasklist` 排除法猜），并明写"加杀软白名单对本案**无效**"；更正模块 docstring 的根因叙述；另修**两处只有实跑工具才会暴露的输出缺陷** —— 长目录名挤爆列宽与"类型"列粘连、持有者行**按 `", "` 切分导致条目被截断**（条目内部本身就含 `", "` ⇒ 改为按结构化行去重）。⑥ **护栏 + 变异验证**：`tests/unit/test_clean_dist.py` 新增 **6** 例（20 → 26：`who_holds` fail-soft、空/具名 `describe_holders`、报告必须具名**且**否定"白名单"建议、docstring 必须写明实测持有者并否掉杀软误判、长目录名不得粘连）；把处置改回"加入信任区/白名单"后新护栏**当场变红**，还原后 **26 passed**（`cmp` 字节一致）。全量快测 **2635 passed / 0 failed**（基线 2629，+6 与新用例数一致）。⑦ **收敛方案（dry-run 实测）**：保留 `dist/` + `dist-electron-out-20260917-142437`（1.1.8），清理 `dist-electron`（1.1.7）；但两者**都被宿主占用** ⇒ 必须**用户**完全退出 WorkBuddy 后 `--apply`（我在宿主进程内**杀不掉它自己**）。⑧ **踩坑存档** `docs/PROJECT_PITFALLS.md` **§二十二**，含"这一幕里工具回执**连续骗人三次**"：`tasklist` 排除法（漏了宿主自身）、宿主读 `.asar` 先报 `Invalid package` 后报 `File does not exist`（**而文件确实在**，`ls`/`find` 双证）、`SHFileOperationW` 对 4 个对照探针返 **`rc=2` 但文件确已被删**。⑨ **遗留（均在用户侧）**：A1 未收敛（需退出宿主）、A4 仍 402 ⇒ 多轮 e2e（`pdf,img`）仍跑不出真实 findings。

**Round 32 (打包产物 e2e 抓出 #132：TEXT 列原样透传 ⇒ 失败页被前端静默丢弃, 2026-09-17)**: 用户手动清空 `dist*` 变体后要求"核对上一轮遗留问题是否已解决 → 满足则重新打包 → 对产物做端到端测试"。① **打包前置核对（只读）**：工作区干净 / 版本真值一致 / Python311+依赖+PyInstaller / Node+electron-builder / `winCodeSign-2.6.0` 已在缓存 / 忽略清单**实测**覆盖全部生成物 / 结构门禁 7 PASS / D 盘 134 G / 无残留进程与端口占用 ⇒ 判定**满足**。唯一坑：Bash 里裸 `python` 解析到 **managed 3.13**（无项目依赖）⇒ 构建必须显式指定 Python311。② **重建产物**：`PyInstaller --clean`（3m19s）→ 后端冒烟 `/health` 自报 `1.1.7` → `electron-builder --win --x64`（29s，**输出到标准路径 `dist-electron/`**，因变体已删故**未触发占用自愈**，也就没有时间戳变体）→ 产物验证：asar 内版本 1.1.7、**无 `PROVENANCE.txt`**（未走自愈）、`extraResources` 与 `dist/pbc-server` **811 文件 / 107.0 MB 逐一致**。门禁 **8 PASS / 0 WARN / 0 FAIL**（`dist_variants` 由 WARN 转 PASS），覆盖率 95.5%。③ **产物级 e2e 抓出 #132**：冻结冒烟 21/0 之后，`#127` 验收从**打包产物**的 `/api/jobs/{id}` 读回 `failed_pages= '[2, 1]'` —— **是字符串**。定位三段：`db/schema.sql` 声明 `TEXT` → `core/pipeline/stage3.py` 写 `json.dumps(...)` → `api/jobs/status.py:71/183` **原样透传**；而 `static/upload.js:739` 用 `Array.isArray()` 取用 ⇒ **字符串不是数组 ⇒ 静默变成 `[]` ⇒ 失败页永不显示**。对照事实：`main.py` 的**复核页**（SSR）早已自行 `json.loads`，所以**有测试覆盖且显示正常** —— 同一字段两张页面行为不一致。#127 专门为"让失败页可见"加的前端渲染就此**形同虚设**。④ **为何长期为绿**：三处护栏各留一条缝 —— `test_api_jobs_coverage.py` 那条断言写死 `'"failed_pages": "[2]"'`（**把缺陷本身固化成契约**）；同文件的字段存在性用例只断言 `field in data`（字符串同样"存在"）；`test_config_error_visibility.py` 与 `tests/e2e_frozen.py:397` 只查**源码文本** `"job.failed_pages" in js`（类型错配与之无关）。⑤ **修法**：`status.py` 新增 `_parse_failed_pages`（沿用同文件 `_parse_*` 的「TEXT → 结构」范式），接入**两个**入口；降级**不伪造 `[]`**（无失败页 → `None`；非法值 → `None` **并记 warning**，否则"数据损坏"与"真的无异常"在界面上不可区分）；`main.py` 改为**复用同一解析器**（call-time 导入，避免与顶层 `from api.jobs import router` 循环）⇒ 同一列不再有两套降级规则。⑥ **护栏 + 变异验证**：新增 6 例纯函数单测（断言**类型**而非取值）、1 例接口级（断言运行时返回数组）、1 例产物级运行时断言 `failed_pages_type`；把两个入口分别临时改回"原样透传"，新护栏**当场变红**且报出实得值（GET `实得 '[2, 1]'`；SSE `实得 ['[2]', '[2]']`），还原后 149 passed —— **实测能失败**，不是"两分支都判 PASS"。⑦ **两处工具陷阱（本轮实测）**：**(a) 同文件并行 `Edit` 互相覆盖** —— 第二条报告"成功"但文件里根本没生效（须**顺序改并回读核对**，工具回执不可信）；**(b) 用字符串探针判断"某模块是否打进 exe"会误判** —— PyInstaller 的 PYZ **是压缩的**，`main.py`/`status.py` 的明文串在 exe 里全部 MISSING；正确判据是**行为证据**（打包产物已在返回该模块产出的字段）。⑧ **升版 1.1.8 并重建**：改动落 `api/jobs/status.py` 与 `main.py`，**都进产物** ⇒ 按约定升版（4 处一致性机检），重建 PyInstaller + electron-builder。踩坑存档 `docs/PROJECT_PITFALLS.md` §二十一。

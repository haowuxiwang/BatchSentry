...
**Current phase**: **v1.1.3** — 在 v1.1.2 之后合入 **R1–R3 降噪**（真实轮次 442→272，−38.5%）、**运行时看门狗**（`core/watchdog.py` + schema v12 的 `jobs.last_activity_at`）、**CI 用例矩阵可逐条审计**，并**重出构建物**（此前产物停在 `8cd0dc1`，落后源码 20 个提交 → 分发它等于发旧行为）。此前 v1.1.2 补完 P0-3 的**服务端转正页面逆映射**（第 8 页 `angle=270`）、**分发一致性机检**与**上传页数上限**。覆盖率门禁 **95%**；真实 51 页全链路 frozen e2e 为放行前提。更早：v1.1 全部里程碑 M2–M7、Phase 12（Feishu job notifications）、2 轮对抗审查，v1.0.0，本地单用户部署（PyInstaller exe + Electron 便携版 win-unpacked）。

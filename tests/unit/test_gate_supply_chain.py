...
def test_cve_reachability_table_covers_snapshot():
    ...
    ⚠️ **零公告是合法终态，但必须显式退役**（2026-09-23，B11-7/B11-21 落地后）：
    依赖全部升到安全版 ⇒ `snap_ids` 为空。此时"双向相等"会退化成
    **两边都空 ⇒ 恒真**，判据失去判别力。旧实现用一句
    `assert snap_ids, "快照里一条公告都没有，一致性断言失去判别力"` 把它挡住 ——
    方向是对的（不许恒真），但**处理方式只是"报红"**：它没有回答
    "公告清零后这张表该怎么办"。

    现在把它**正面规定**出来：清零时，可达性表必须**显式退役**
    （文件里要有"已退役"声明，说明日期与原因），且那一支**不是 return 掉就完事** ——
    声明缺失必须红。这样文件层面才分得清：
      · "**全部修好了、表已按流程退役**"（本支通过）
      · "**表没人管了 / 悄悄留白**"（本支变红）
    一旦**新公告**出现，`snap_ids` 非空 ⇒ 落回下面的双向相等 ⇒ 强制逐条重新判定。
    """
    ...
    if not snap_ids:
        doc = (REPO / "docs" / "CVE_REACHABILITY.md").read_text("utf-8")
        assert "已退役" in doc, (
            "依赖快照已无公告，但 docs/CVE_REACHABILITY.md 里**没有任何退役声明** ⇒ "
            "无法区分「公告已全部修好、表已按流程退役」与「表没人管了」。\n"
            "请在该文件顶部写明：本表已退役 + 日期 + 原因（依赖已升到安全版本）。"
        )
        return

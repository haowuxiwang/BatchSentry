...
>
> 最后更新：2026-09-18（Round 40：**第二轮对抗性审查（基线 = v1.1.9）** ——
> 逐条复核上一轮的 8 项指控；用**多模态直接读第 8 页原图**做交叉比对，确认
> 「OCR 把 `4.6` 读成 `46`」并**验证当前版本已有一等对策**（`_decimal_loss_factor`，
> 直接调用复验 info/warning 分级正确）；新增 6 条有效发现；**判定：无 P0 阻塞，
> v1.1.9 可作为分发基线**；产出 **v1.2.0 迭代清单**（见文末）·
> 审查报告 → `docs/ADVERSARIAL_AUDIT.md` 的「第二轮对抗性审查（Round 40）」）·
> 可分发版本 **v1.1.9**（**11 条缺陷已修且产物已重建**）·
> 远端 `ac42202`（Round 39 推送成功）·
> **A1 已解除**（用户已手工收敛产物目录）
...
- [ ] **B0-1 给 v1.1.9 打 tag 并写分发说明**
  证据：版本四处一致（`main.py:30` / `package.json` / `package-lock.json` 两处 / `PORTABLE_README.txt:104`）、
  `dist-electron` 唯一、门禁 8 项全绿（95.24%）、asar 与内嵌 exe 均已实跑。
  动作：`git tag v1.1.9` + push tag；确认 `DEPLOYMENT.md` 记录了本轮产物路径与本次"分发判定"结论。
  验收：`git tag -l 'v1.1.*'` 含 v1.1.9；远端有该 tag。
  备注：历史 tag 已补到 v1.1.2（#98），v1.1.3–v1.1.8 的 tag 情况需一并核对补齐。

- [ ] **B0-2 记录"分发基线"的诚实边界**
  动作：在 `docs/ADVERSARIAL_AUDIT.md` §14.4 已列明；再在 `DEPLOYMENT.md` 合并一段
  「本基线的证据边界：本机实测 / 未做他机验证 / GUI 需人工双击」。
  验收：新接手的人只看 `DEPLOYMENT.md` 就能知道**哪些结论未被验证**。
...
- [ ] **B2-5 #127 全局性故障仍呈现为绿色成功**（P1，**历史登记未修**）
  现象：LLM key 失效 ⇒ 页级 `_parse_error` ⇒ `partial_review` 时 `jobs.error_message` 恒 NULL
...
- [ ] **B4-1 SSE 断点续传与心跳**（P2，本轮新发现）
  现状：仅有 `retry:` + `data:`；未见 `id:` / `Last-Event-ID`，也未见心跳注释帧。
  动作：加 `id:`（事件序号）+ 支持 `Last-Event-ID` 续传；加周期性 `: keep-alive` 注释帧。
  验收：断线后重连能续上不重放；长 job 经代理不超时。

...
- [ ] **B11-1 守卫前移到 ASGI 中间件 + 请求体大小硬上限**（对应 **D3**／**P1**／计划 §SEC-1）
...
- [ ] **B11-2 门禁新增第 10/11 项：依赖漏洞 + 运行时 EOL**（= B10-4／**P1**／计划 §SEC-4）
...
- [ ] **B11-3 `dist_variants` 判据升级**（= 计划 §HYG-2／W1）
...
- [ ] **B11-4 S2 逐条 CVE 可达性表 + `npm audit`**（= B10-2 的边界／计划 §SEC-5／W1）
...
- [ ] **B11-6 图片路径专项用例先写好**（= 计划 §FUNC-4 的准备／W1）

...
- [ ] **B1-12 布尔/勾选类的处置档位由「措辞是否撞上词表」决定**（P2，Round 46 发现）
  现象：同是"勾选了否"，`p39`/`p46` 走 **L1 抑制**，而 `p45` 只降级 —— 因为
  `_NOT_A_VALUE` 词表含"不符合操作指导" ⇒ `_declared_spec_value` 返 `None` ⇒ fail-open。
  即处置档位由**文案措辞**而非**问题性质**决定。
  ⚠️ 且 **p45/p46 从未做视觉核实**（Round 43 只核实过 p39，其结论是"是/否框
  **全部勾的是「是」**"）⇒ 这两条里的"否"究竟是不是真"否"，**目前无证据**。
  动作：① 先读原页把 p45/p46 的勾选真值定下来（**先定位再改**）；
  ② 让布尔类判定走**统一的类型闸门**，而不是依赖措辞词表 —— 与 B1-5 的
  "形态闸门"同源。

- [ ] **B1-13 【P2】时间倒序判据缺"行级溯源"⇒ 跨行串位只能降级、无法有据抑制**（Round 48 从 B1-5 拆出）
  现象（B1-5 现象 B 的残留）：p38 `llm_cross`「清洗罐搅拌结束 18:30 早于开始 19:15」——
  视觉核实原件为 **开始 19:04 / 结束 19:41**（顺序正常），LLM 拿的是同页**另一行**
  「退出循环」的 18:20/19:15。该条现在是"判不了 ⇒ 降级为 warning"，**不是**有据抑制。
  定位（Round 48 实测，`devlogs/_replay/` job `6f80145a`）：p38 的
  `structured.steps` **start/end 全为空串**（仅 3 段长文本工序），raw 原文里
  **不存在** 19:04/19:41 ⇒ **该页没有任何行级证据**可证伪 LLM 的时间。按工序名定位
  也不可行（step `operation` 是段落文本，与文案的「清洗罐搅拌」对不上）。
  ⇒ **不是判据写得不够，是输入里没有可用的行级锚点。**
  动作：① 让 LLM 在抽取时**回填 `source_row`/`field`**（改了输出 schema，需同步
  `core/page_analyzer.py` prompt、`llm_checks`、`llm_finding_guard` 三处）；
  ② 判据改为"校验两个时间来自**同一行/同一字段组**"，无法回填 ⇒ 降级（现状即降级）。
  验收：构造"两个时间来自不同行"的 fixture ⇒ 抑制；构造"同一行确有倒序"⇒ 保留。
  变异验证：把行级校验改回"全页任一工序倒序即可"⇒ 跨行串位用例红。
  ⚠️ 优先做 ① 的**代价评估**（schema 变更会同时影响 prompt 版本号、金标样本、
  `tests/unit/test_llm_checks.py` 一类契约测试），**先出方案再动**。
  ⚠️ 与 **B1-2（`vision_crosscheck`）** 天然互补：视觉读到的 19:04/19:41 vs OCR/LLM
  的 18:30/19:15 正是"待人工核对"的触发条件 —— 若 B1-2 落地，本条的收益会下降。

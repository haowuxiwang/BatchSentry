...
## R1–R3（评审 `ARCHITECTURE_AND_QUALITY_REVIEW.md` §5）—— 已完成

> 上游依据：该评审 §2.3 的真实轮次噪声拆解（自指元噪声 71/347 = 20%、批号 5 组假警报、
> 2015 日期级联 12 条含 3 critical）。
>
> **基线口径说明**：评审引用的 `95a27d88-52b` 轮已随 `%TEMP%/pbc_e2e_appdata` 隔离库清理，
> **不可复算**（见 `docs/ADVERSARIAL_AUDIT.md` §10）。本轮所有量测改用**仍保留**的
> 真实 51 页轮次 `0c5cfb00-897`（Paddle，442 条 findings）。

### ✅ R1 消灭自指元噪声（已完成）

- **定位依据**：真实轮次 `handwritten` 65 条 + `整体识别置信度较低` 7 条 = **72/442 = 16%**。
  文案分别是"本页含 N 条手写内容…请人工核对"与"整体识别置信度较低…建议人工核对 OCR 结果"
  —— 讲的是**工具看不清**，不是**记录有问题**，却**计入 finding 计数**。
- **改动**：`core/finding_noise.py` 新增 `reduce_self_referential_noise`（与 M2 的
  completeness 降噪**分开** —— 前者按文档属性聚合，后者按结构缺失 kind 细分）；
  `stage3.py` 在 M2 降噪**之前**接入。整份文档聚合成 1 条，携带条数与**完整页清单**。
- **与评审方案的差异（已标注）**：评审写"移到页级横幅 / 汇总区"。本轮实现为
  **文档级摘要 finding**（`aggregated=True` + `page_list`），**未改前端横幅数据通路**
  —— 横幅当前由 `page_cache` 诊断驱动、不由 findings 驱动，把自指信息接进横幅是
  另一处改动（列为后续项）。信息未丢失：摘要携带全部页号。
- **机检不变式**（`tests/unit/test_finding_noise.py`）：`handwritten` 必须在
  `CANONICAL_TYPES`（聚合后仍用它，六面同步才不漂）；文案契约与 `rule_doc.py:370`
  双向绑定（文案改了测试立即失败）；<2 条不聚合；幂等（二次跑不继续聚合）；
  输入不被修改；页级证据闭合（原始页 ⊆ 降噪后页 ∪ 摘要页清单）。
- **实测**：442 → **372**（−70；72 条 → 2 条摘要）。`critical` 29 → 29。

### ✅ R2 跨页批号一致性改投票归一（已完成）

- **定位依据**：规则层重放同一轮次报 **"5 组不同批号"（19 个原始变体）**，但逐组看是
  **同一批号的 OCR 写法**：`1127011N250101` / `1127011N^*250101` / `11270111/250101`
  —— 差异要么是纯符号（`^` `*` `/`），要么是单字符（`N`↔`1`）。
- **改动**：`parsing._normalize_batch_no` 增加**符号剥离**（除 `-` 外全部非字母数字，
  `-` 留给工序后缀切分）；`rule_doc._check_batch_consistency` 增加**投票**：多数核心串
  覆盖 ≥ `_BATCH_VOTE_MIN_SHARE`(0.6) 的有批号页时，把**编辑距离 ≤1** 的组吸收为变体，
  吸收数量**明示**在描述里。新增 `parsing._edit_distance_le1`。
- **保守性**：投票不决定性 / 距离 >1 → 照旧全报。`B202201` vs `B202202`（各 1 页，
  2 页文档）投票不决定性 → **仍报 critical**（真实混批绝不被吃掉）。
- **机检不变式**：符号剥离与 `-` 保留语义；非决定性全报；吸收必须明示；
  `_edit_distance_le1` 的 8 个边界（含长度差 >1 直接否决）。
- **实测**：**5 组 → 2 组**（19 变体，2 个被投票吸收）。finding 条数不变（仍是 1 条，
  但**内容从"5 组"变成"2 组"**，复核员不再被假警报带走）。

### ✅ R3 日期歧义交叉约束（已完成）

- **定位依据**：`year_contradiction` 里 `[2015, 2025]` 形态，少数年份与多数年份
  **只差一位数字**，且页支持度远低于多数 —— 是同一次单字符误读在多页重复。
  更糟的是 R1-b/R5/R9a 用 `year_delta > 2` 把同一次误读**再报一遍**（2015 级联）。
- **改动**：新增 `core/rules/year_vote.py`（**纯函数、无 IO、无状态**）：
  统计全文档年份页支持度 + 批号 `YYMMDD` 段作为**独立佐证**；`resolve()` 需**五道闸
  同时成立**才归一 —— ① 有佐证（批号佐证，或少数读法只出现在 1 页）；
  ② **不相邻**（|Δ| > 1，保护跨年记录）；③ 单字符差异；④ 多数基数 ≥3 页；
  ⑤ 投票决定性（少数页数 < max(2, 多数页数 × 0.35)）。
  接入 **4 个发射点**：R2 `_check_year_contradiction`、R1-b `_check_time_reversal_cross_page`、
  R5 `_check_signature_time_anomaly`、R9a `_check_signature_order`。
- **不做**：**不改写任何原始文本**（不重写日期串）—— 只在规则发射点抑制误读派生的结论，
  原始 OCR/LLM 产物与证据锚一字不动。
- **机检不变式**（`tests/unit/test_year_vote.py`）：五道闸各自有否决用例；
  **相邻年保护**（2024/2025 永不被归一）；差 ≥2 位不归一；批号佐证的年份永不被吞；
  平票时无多数（不归一）；确定性；`date_strings` 与 `rule_time._collect_all_date_strings`
  **口径等价**（防漂移）；四个发射点各有"归一→抑制"与"不归一→照旧发射"双向用例。
- **实测**：`year_contradiction` **9 → 3**（−6）。其余 11 个类型零变化。
- **已知限制（诚实标注）**：`[2018, 2025]` 这类**差 2 位**的保持原样 —— 它很可能是**真实的
  文件模板年份**（文件 2018 年审核 vs 批记录 2025 年生产）。**这不是"未修完"，而是
  不该被归一**：真正的修法是让 R2 区分"文件版本审核"与"批记录复核"两个不同对象，
  属 `event_type` 粒度问题，需 R4 标注集才能判定改得对不对。

### R1–R3 净效果（可复现）

```bash
PY="C:/Users/WuSiTan/AppData/Local/Programs/Python/Python311/python.exe"
"$PY" scripts/replay_rules.py        --job 0c5cfb00-897 --db <data.db>   # R2/R3：432 → 426
"$PY" scripts/eval_noise_reduction.py --job 0c5cfb00-897 --db <data.db>   # R1/M2：442 → 272
"$PY" scripts/eval_findings.py                                            # 漏检对照：P/R/F1 = 1.0
```

| 阶段 | 总数 | 噪声占比 | critical |
|---|---|---|---|
| 原始 | 442 | 75.8% | 29 |
| R1 自指元噪声聚合后 | 372 | 71.2% | 29 |
| 再叠加 M2 completeness 降噪 | **272（−38.5%）** | **60.7%** | **29（±0）** |

高价值类型（`param_out_of_spec` / `time_reversal` / `batch_inconsistency` /
`signature_time_anomaly` / `self_review`）**一条未减**；页级证据无丢失；
`eval_findings.py` 的 `P/R/F1 = 1.0（TP=10 FP=0 FN=0）` —— **降噪不是靠丢真命中换来的**。
...
| T-P0-3 区域级证据锚（含宽高比闸门 + 旋转逆映射） | ✅ 完成（含方向独立核验） | v1.1.1 + v1.1.2 |
| **R1 消灭自指元噪声** | ✅ 完成（实测 −70 条） | 本轮 |
| **R2 跨页批号改投票归一** | ✅ 完成（5 组 → 2 组） | 本轮 |
| **R3 日期歧义交叉约束（年份投票）** | ✅ 完成（9 → 3 条） | 本轮 |
| T-P0-1 选择性双读仲裁 | ⬜ 未开始（前置 T-P1-3） | — |

**R1–R3 已开工并完成第一轮**。降噪总效果 **442 → 272（−38.5%）**，
`critical` 与高价值类型零损失，漏检对照 P/R/F1 = 1.0。
...
**但真正的卡点是 R4 标注集** —— 在那之前，`completeness` 剩下的 164 条里哪些是
真缺失、哪些是模板属性，**无法判定**（见 `docs/ADVERSARIAL_AUDIT.md` §11）。

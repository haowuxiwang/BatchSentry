...
- ⚠️ **口径更正（2026-09-21 Round 53，B1-10）**：上面这行"5 组 → 2 组"是在 job
  `0c5cfb00-897` 上测的；**抽取行为变化后该结论回归** —— 在 job `6f80145a-47f`
  （51 页真实批记录）上，日志明写 **`0 voted`**，即投票**一次都没触发**，仍报 5 组。
  根因有二：① 旧归并按**核心串长度**降序用前缀包含互相吸收 ⇒ 噪声串 `1127011N4250101`
  成了 host（组标签即噪声）；② 投票以**精确核心串**计页数，最大核心串只占
  27/46=58.7% < 0.6 ⇒ 14 页"尾段被丢"的截断读法把票数劈开 ⇒ 不决定性 ⇒ 不吸收。
  ⇒ 同一条判据在不同抽取产物上**可判性不同**，这正是 PITFALLS §二十三"判不了 ≠ 判据确凿"
  的实例：**判据写对了不等于在真实数据上有判别力**。
  已按**结构优先、无顺序依赖**重写（`parse_batch_key` 拆 `(prefix, date6)` +
  两档兼容关系 + 锚点按页数选），实测同库回放 **5 组 → 2 组**、基准标签纠正为
  `1127011N250101`；存活的 1 组是 p50 `1127011N 241203`（原图核验为**真实值**，
  清场记录引用他批）⇒ 详见 `docs/TODO.md` B1-10 与另立的 B1-14。
...
| **B1-1 同根因 finding 聚合**（发射期合并，见下） | ✅ 完成（规则层 426 → 406，−20；critical 零变化） | 2026-09-21 |
| T-P0-1 选择性双读仲裁 | ⬜ 未开始（前置 T-P1-3） | — |

### B1-1 同根因聚合（与 R1–R3 **不同轴**的降噪）

R1–R3 做的是"**类别级**"降噪（在**已落库**的 finding 集合上，按年份/批号/自指
等根因抑制）；B1-1 做的是"**发射期**"降噪：单个 OCR 误读会在**每个时间点各产 1 条**，
`rule_spec` 逐值判定且全链路无聚合层。

- **机制**：`core/rules/finding_aggregate.py` —— 判定点以**唯一构造点**
  （`make_oos_finding`）产出 finding，并同时挂上**不落库**的派生字段
  （`_agg_parts` 结构化部件 / `_agg_key` 分组键）；`aggregate_by_root_cause`
  按派生键分组，**组内 ≥2 才合并**，单条逐字节原样通过。
- **聚合键** = page·type·kind·主语·**根因**·取值·单位·规格·提示语；
  **位置（时间点）是变化维度，故意不进键**。
- ⚠️ **不得用描述文本反推键**：粗口径（page+type+前 60 字）在该语料上会给出
  "12 组可合并"，其中多数是**假合并**（"缺少 QA 签名" vs "缺少复核人签名" 是
  不同根因）。派生键是这条纪律的落地方式。
- **留痕**：合并后 severity 取最高；`ocr_text` 拼接全部成员明细；返回**明细行**
  （组键/成员数/成员原文/severity 分布）而非计数。
- **实测**（`devlogs/_verify/replay_b11.py`，官方 `replay_rules.py` 同链路，
  仅切聚合开关）：job `0c5cfb00-897` **426 → 406**（−20；p8 ×7、p33 真空度 ×8 ×2）；
  job `6f80145a-47f` **493 → 487**（−6）；单条文案与生产库**逐字节一致（7/7）**。
- **变异验证 12/12 CAUGHT**（`devlogs/_verify/mutate_b11.py`），其中含"入口不接线"
  与"聚合键漏字段"两类最易假绿的情形。

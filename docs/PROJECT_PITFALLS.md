...
- ⚠️ **引用纪律：别把借来的数字当自测值。** 曾把「云厂商 1–2 s/页」（凭印象编的）写进 8 处
  文档/代码，还配了"快 10~30 倍"的因果叙事 —— 反而更易混过自查。事实是**价格**差 10~30 倍
  （纯文本抽取 $1.50/1,000 页 vs 结构化/生成式 $10~50），各厂商**不公布延迟**。
  **引用外部数字必须标出处 + 分清量纲（价格≠延迟）**；护栏
  `test_vendor_latency_claims_are_forbidden`。

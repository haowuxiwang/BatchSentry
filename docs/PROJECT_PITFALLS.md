...
## 四十、"判据自证"的五类新陷阱（Round 59 W1，2026-09-23 实测）

> W1 干的活是"给分发门槛补机检"。过程中**一半的时间花在纠正自己刚写的判据**上：
> 探针给错结论、计数虚高、变异脚本自己掩盖了 MISS。以下全部是**本轮实测**。

### A. 🔴 没有**正对照**的探针，结论一律不可采信

`devlogs/_verify/probe_pillow_surface.py` 首轮输出"Pillow 可读格式数 = **0**"，
连 `PNG` 都报 `openable=False` —— 而产品明明每天在解码 PNG。
**原因**：`PIL.Image.OPEN` 是**惰性**填充的，不先 `Image.init()` 就读到空表。
加了正对照（塞一张真 PNG，必须被认领为 `PNG`）后，真值 = **44 个格式**。

📌 **推广**：凡探针得出"什么都探测不到/什么都不支持"，**先证明探针能认出一个已知良好的样本**。
"探测不到 X" ≠ "X 不存在"（同 MEMORY：探针本身会骗人）。
本轮若照首轮结论写文档，会得出"Pillow 面完全不可达"的**假安全**结论。

### B. 🔴 先假设会错：**内容嗅探**推不出"格式白名单无效"

本轮初始假设："`Image.open` 按**内容**嗅探 ⇒ `_IMAGE_EXTENSIONS` 不构成格式约束 ⇒
JP2/GD/PDF 都能借道打进危险解码器"。
**被证伪**：`api/jobs/upload.py:160-181` 在**解码前**有 **magic bytes 白名单**
（`api/jobs/__init__.py:94-101`：JPEG/PNG/BMP/TIFF/WEBP）。把头拦掉后，
Pillow 的可达面从"很多"压到**只剩 TIFF**（16 条里 1 条可达）。

📌 **推广**：推理链的**每一步都要有一处代码/实测**兜底。"按内容嗅探"只是
`Image.open` 的行为，**不等于**"没有别的闸门"。差点据此写出一张错的可达性表。

### C. 🔴 同名 ≠ 同物：按**包名**匹配会得出错误结论

`npm audit` 报 `nanoid` 受影响（`<3.3.18`）。我一度记为"**随 `docx` 分发的**
`nanoid` 有洞"。核对 `nodes` 字段后真相是
`node_modules/postcss/node_modules/nanoid@3.3.16`（**构建期**依赖链），
而**随包分发**的那份是 `nanoid@5.1.16`（**不在**范围内）。

📌 **推广**：判"某个依赖有没有洞"必须落到**具体安装路径 + 具体版本**。
一个仓库里同名包可以有多份副本，分属**不同的分发层**（运行时 / 构建期 / 随包死代码）。
顺带一条同源教训：`grep -c node_modules app.asar` 返回 402，看起来"asar 里有 node_modules"，
但那是**二进制里恰好出现的字符串**；真值要**解析 asar 头的 JSON 文件树**（203 条目 / 201 在 `node_modules/`）。

### D. 🔴 计数类判据会**虚高**：把"同一条被列 2 份"当真

`pip-audit -r requirements.txt` 会在**同一个包的 `vulns` 里把同一条公告重复列出**
（实测**每条正好 2 份**）。照抄 `len(vulns)` ⇒ 把 **24 条唯一公告报成 47 条**，
严重度虚高 ~2×。pip-audit 自己的汇总行（"Found 47 known vulnerabilities"）也是这个虚高值。

📌 **推广**：聚合类判据要么**按唯一键去重**，要么**同时报两个数**（唯一 + 原始）。
去重不是"把信息藏起来"：本轮同时保留 `raw_entry_count`，使两者都可见。
⚠️ 注意：**去重不改变安全结论**（`minimum_safe_version` 取最大修复版本，与重复无关），
只污染**证据链与人工判断** —— 但"只影响汇报"正是它危险的地方：没人会去复核一个数字。

### E. 🔴 变异脚本自己会**掩盖 MISS**（三类，全部实测踩到）

1. **多行锚点遇 CRLF ⇒ 0 次匹配**：本仓库 `api/jobs/upload.py` 是 **453 个 CRLF / 0 个裸 LF**，
   锚点写死 `\n` 会 ANCHOR-ERROR（首轮 M4 即如此）。
   ⇒ 按该文件**实际换行符归一化**锚点。
2. **参数化用例同名多实例**：用"最后一个实例覆盖"的写法，会让"**只杀掉其中一个参数**"的
   变异被掩盖、假报 **MISSED**（实测：M3 只让 `bad=0` 那一个实例变红）。
   ⇒ 聚合规则必须是"**任一实例红即整体红**"。
3. **`must_fail` 里写了与变异无关的用例** ⇒ 假报 MISSED。
   这不叫"判据失效"，叫**期望写错**（同 §三十：断言落在同名标识的别处）。
   ⇒ 列 `must_fail` 前先确认"这条用例**确实会走到**被变异的那段代码"。

📌 **推广**：变异验证是"判据的判据"，因此它**自己**也需要对照组：
基线必须全绿（否则结论无意义）、锚点必须唯一且匹配成功、
被点名的用例必须真能覆盖被变异的分支。**"CAUGHT" 只有在基线绿、锚点命中、用例名拼写正确时才可信。**

### F. 收尾：本轮"看着在守、其实守不住"的两个实例

- **断言过弱的 magic 用例**：原断言 `"图片" in r.text` —— 而"进了解码器才失败"
  也返回含"图片"的 400 ⇒ **摘掉 magic 闸门它照样绿**（零判别力）。
  改为断言 **"文件头"** 后，变异 M5 才能把它打红。
- **只测"比较逻辑"的像素上限用例**：原用例把上限 `monkeypatch` 到很小、用一张小图，
  只证明"比较对"，**没有**证明"声明超大尺寸的文件会在**解码前**被拒"。
  补了**头部炸弹 PNG**（真实像素 8×8、IHDR 声明 40000×40000，仅 78 字节）才测到真行为 ——
  并就此挖出 `DecompressionBombError` 没被接住 ⇒ **500** 的真缺陷。

📌 **推广**：判据要问"**它能不能红**"，更要问"**它测的是不是我以为的那件事**"。
"用例存在" ≠ "用例在守那件事"。


## 四十一、"预演升级"的六类陷阱（Round 59 W2 预演，2026-09-23 实测）

在**隔离 venv** 里预演 `Pillow 10.1.0 → 12.3.0`（= B11-7 的目标版本）时踩到的。
**六个都不是产品缺陷**，而是"**验证动作自己**"的陷阱 —— 但每一个都足以让我
得出**方向相反**的结论（比如把"还没跑到"读成"参数没生效"）。

### A. 读"中间态产物" ⇒ 把"没跑到"当成"没生效"

现象：`python -m venv --system-site-packages <dir>` 之后读 `pyvenv.cfg`，得到
`include-system-site-packages = false`，而且 `command` 行里**连这个参数都没有**
⇒ 第一反应是"参数被吃掉了"（本机确有 `curl -w '%{http_code}'` 被吃掉的先例，形态很像）。
**真因**：CPython 的 `venv` **先写 `false`、再在最后回写 `true`** ——
`Lib/venv/__init__.py:69-84`：先 `self.system_site_packages = False`（issue 24875，
为了让 `ensurepip` 装自己的 pip），全部装完后 `if true_system_site_packages:` 再
`create_configuration()` **重写**一遍。**我的创建过程被外层 120 s 超时 SIGTERM 打断在回写之前。**
⇒ 读到的不是"参数没生效"，而是"**还没执行到那一行**"。
📌 **推广**：**产物是分阶段写出的，"读得早"与"写坏了"长得一模一样。**
判"某设置是否生效"之前，先确认**产生它的流程真的跑完了**（本例：把 `VENV_RC`
与 `Scripts/pip.exe` 在不在一起作为"跑完没"的判据）。
与 §三十三（并发改写下读到变异中间态）同源：**中间态不是终态**。

### B. 长命令别放前台：外层超时会把"半成品"留在磁盘上

`python -m venv` 在本机（git bash + 杀软）实测 **>120 s**（含 `ensurepip`）
⇒ **前台跑必然被工具默认超时 SIGTERM**。而它**不是原子操作** ⇒ 磁盘上留下一个
"**有 pip、没有 activation 脚本、cfg 还是 false**"的半成品（A 里读到的就是它）。
⇒ **凡新建 venv / 装包 / 构建，一律放后台**。别用"调大 `timeout`"去赌：
我写了 `/usr/bin/timeout 300`，**外层 120 s 先到**，那个参数**根本没机会生效**。
📌 **推广**：**超时要打在最外层。内层加超时不影响外层先杀。**

### C. `--clear` 会被 safe-delete 批量阈值挡下；而**父进程不继承**你注入给子进程的阈值

两个连着的坑：

1. `python -m venv --clear <dir>` 需要删 **6694 个目标**（阈值 **50**）⇒ shim 拒绝、`RC=1`，
   输出 `[safe-delete][SAFE_DELETE_BULK_CONFIRM_REQUIRED] {"count":6694,...}`。
   ⇒ 删不掉就**换个新路径重建**，或显式放宽阈值（`CODEBUDDY_SAFE_DELETE_BULK_THRESHOLD=100000`）。
2. 更隐蔽：我给 **pytest 子进程**注入了该阈值，**父进程（变异脚本自己）没有** ⇒
   脚本里那句 `xml_path.unlink()` 直接让**整个脚本静默中止**：
   **`RC=1`、没有 Traceback、只多出一行 `SAFE_DELETE_BULK_CONFIRM_REQUIRED`**；
   而我的 `print` 输出因**块缓冲**被冲到了**文件开头/末尾之外**的位置 ⇒
   看起来像"基线跑完就没了"，很容易误读成"逻辑写错了"。
   ⇒ 修法**不是**"再设一次阈值"，而是**根本不删**：junit 每次用**唯一文件名**
   （`_mutate_pillow_run01.xml`…）—— 顺带多了一条"哪个变异号对应哪份 junit"的可追溯性。
📌 **推广**：① **能不删就不删**（唯一文件名 > 清理旧文件）；
② 注入环境变量时要问"**这份配置对谁生效**"（父/子是两个进程）；
③ **输出顺序会骗人**：stderr 无缓冲、stdout 块缓冲 ⇒ 混排后"谁先谁后"完全不可信，
   读日志要按**内容**判因果，**别按行序**。

### D. "变异被打不红"要先分三种：判据漏了 / 期望写错 / **变异等价**

M5（把 `if pinned < (since,)` 改成 `<=`）首轮 **MISSED**。第一反应自然是"边界断言写漏了"。
**实际是"变异等价"**：`since = 12` ⇒ 比较对象是 `(12,)`，而**元组比较里元素相同则"长的大"**
⇒ `(12,0,0) > (12,)`、`(12,3,0) > (12,)` ⇒ 在我给的输入上 `<` 与 `<=` **完全等价**，
**没有任何输入能把两者分开** ⇒ 判据**无从打红**。
差别只在**退化形态** `pinned == (12,)`（= `Pillow==12`，只有主版本号）时才出现。
补上这条退化输入后 **M5 → CAUGHT**（终态 **7/7**）。
📌 **推广**：变异没被打红，**不等于**判据失效。先问"**这份变异在给定输入上是不是恒等变换**"。
是 ⇒ 补一个**能把差别暴露出来的输入**（本例：退化形态），或承认它是等价变异并**具名记下来**；
不是 ⇒ 再回头查判据 / 期望。（与 §四十 F 的"期望写错"三分类并列。）

### E. "隔离预演"必须先证明隔离成立

在 venv 里升依赖，**唯一目的**就是"不动用户环境" ⇒ 所以"**没动**"必须是**实测**的：
升级后逐项对照 venv 与全局 Python311 的 4 个包版本（venv 新 / 全局旧），
才敢说"隔离成立"。否则这个预演**本身就是一次环境事故**。
📌 **推广**：**"我用了隔离手段"不是隔离成立的证据**（同 §三十八 D8 的 `bootLog`：
用了 `app.getPath('appData')` 就以为隔离了）。**要拿两侧的实测值对照。**

### F. 采样环境不同 ⇒ 耗时不可比，更不可当性能结论

同一套用例：全局 Python311 **6m09s**；新建 venv **12m37s**（**2×**）。
差因**未归因**，但"**新建 venv 首次导入 + 杀软扫描**"足以解释。
⇒ **不得**据此写"升依赖导致性能回归"；B11-5 的吞吐判据**仍须在新鲜产物上做**。
📌 **推广**：**换解释器 / 换目录 / 换机器之后，"变慢了"先怀疑采样环境**（§三十三同源）。
性能判据必须**同环境对照**；跨环境只可作**方向性参考**，不可作结论。

## 四十二、"验收驱动"的假绿：把"成功终态"当成"链路已验"（Round 59 W3，2026-09-23 实测）

**这是本项目迄今最有价值的一次"判据自己骗自己"** —— 前六轮审查、门禁 11 项、
63 条护栏**全部没抓到**，只有**真跑一次端到端**才暴露。

### A. 现象：门禁放行了一条从未被验证的链路

D4（LLM/OCR 成功路径真验）第一次跑通后，覆盖清单写着：

```
[covered] llm_pipeline —— 流水线到达成功终态（terminal=review）
```

`REQUIRE_LLM=1` ⇒ `required_gaps` 只看 `llm_pipeline` 是否 `covered` ⇒ **退出码 0**。
但**直查库**（不采信驱动自述）立刻翻船 —— `llm_call_audit` 显示 **3 条调用里 2 条失败**：

| stage | success | 详情 |
|---|---|---|
| `page_analysis` | 1 ✅ | 3213+3126 tokens / 72837 ms |
| `cross_page_llm_fallback` | **0** ❌ | `402 code=30001` |
| `cross_page_llm` | **0** ❌ | `402 code=30001` |

而 findings 的描述里**白纸黑字**写着「LLM 调用失败: RuntimeError」。
即：终态 `review` 是**降级路径**达成的，**LLM 兜底从未成功**。

### B. 根因：一个从未落地的蕴含

`classify_pipeline` 的规则只有一句：
...
它把「**到达成功终态**」当成「**该链路已验**」。这个蕴含**不成立**：
产品在 LLM 失败时走**降级**，而**降级照样能产出 `review`**。
⚠️ **讽刺的是，这条警告是本项目自己写在 B9-7 里的**（"成功的终态**不蕴含** LLM 被调用过"）
—— 但**它只被用在"没配凭据"这一种情形上**，从未用于"配了凭据但调用失败"。

### C. 五类推广（判据设计的通用纪律）

1. **「跑过」≠「跑通」｜「终态成功」≠「链路已验」。**
   终态是**产品**的结论，不是**链路**的证据。带降级的产品必须把
   "**降级是否被触发**"当成一等公民来记账。
2. **证据要取产品自己暴露的权威判据，别在驱动里另读一遍库。**
   本项目 `GET /api/jobs/{id}/llm_audit` 就是权威入口（`api/review.py:715`）。
   绕过它去查 SQLite ⇒ 造出**第二份实现**，两份必然漂移。
3. **「凭据被写入」≠「凭据被接受」≠「链路跑通」—— 三者必须分开记账。**
   `llm_config`（配置通路）/ `llm_pipeline`（执行通路）是两个条目，不可互相顶替。
4. **判断值禁止由被测对象推导。** 假绿可以**从另一个入口回来**：判据改对了，
   但接线处用 `terminal` 推导证据 ⇒ 同样的洞。本项用 **AST 判据**钉住：
   调用点必须传 `llm_call_succeeded` / `ocr_backend_used`，且这两个值
   **不得**引用终态/状态字段。⚠️ 同步的陷阱：**护栏自己会把合法的 `upload_failed`
   分支误判成违规** ⇒ 规则要窄到"**断言了真实终态的调用点才须传证据**"，
   并**给豁免面写阳性对照**（否则护栏逼着你把 fail-closed 分支写坏）。
5. **修复只在"能红"时才算验完。** 判据修好后，**必须用同一个真实场景复跑**，
   并确认它**变红**：本次 `E2E_RC` 由 **0 → 1**，且点名 `llm_pipeline=failed`。
   "修复后单测绿"**不算证据** —— 单测是我写的，真场景是外部给的。
   （变异验证 **7/7 CAUGHT** 亦已补做。）

### D. 外部故障与产品缺陷必须**可区分**，且"上次成功"不保证"这次成功"

同一把 key：先前一次 `chat` 探针 **HTTP 200**；首次 e2e 的 `page_analysis` **成功**
（消耗掉了账户最后的余量）；此后**全部 402**。逐模型实测（含用户点名的三个模型）：

| 状态码 | 含义 | 归属 |
|---|---|---|
| **401** `code=30014` Token is invalid | 凭据无效 | 历史事故的成因（**产品侧**要查提供方/端点是否错配）|
| **402** `code=30001` balance insufficient | 账户无余额 | **纯外部**，与产品无关 |
| **200** | 可用 | — |

⇒ ① **必须把 402/401/超时/格式错误分开报**，否则"没钱"会被误记成"产品坏了"；
② **凭据可用性必须每次实探**，不可引用上一次结论（同一把 key 在同一天内由 200 变 402）；
③ ~~免费档模型**不存在于本账户**~~ —— 🔴 **本条已被证伪**（同日充值后
`DeepSeek-V3.2` / `Qwen3.5-35B-A3B` / `V4-Pro` / `V3.1-Terminus` **4/4 实测 200**）。
真因是**余额为 0**（余额为 0 时**连免费档也被拒**），不是"该账户不支持免费模型"。
⚠️ **教训**：**不要把"当下没钱"这种瞬时状态写成"该账户不具备某能力"这种结构性结论**
—— 后者会让后来的人据此放弃尝试。**结构性问题与瞬时问题必须分开写**。
📌 **推广**：**凡"环境/额度"类失败，先证明它不属于被测对象**；报结论时
**必须写明归因**，别让读者自己猜是在骂产品还是在骂账户。


---

## 四十三、"真验"的四个新陷阱（Round 59 W3 D4 真跑，2026-09-23 实测）

D4（LLM + OCR 真成功路径）从"从未真验"到"三层互证通过"，过程中连翻两次车。
四次都是**验收方法本身**的缺陷，不是产品缺陷 —— 但**每一次都差点被写成产品缺陷**。

### 一、探针的**请求规模**不等于被测请求 ⇒ "外部正常"这个归因可能是错的

实测（同一把 key、**同一时段**）：

| 请求 | 形状 | 结果 |
|---|---|---|
| 直连探针 | `max_tokens=16`，数词消息 | **HTTP 200** |
| 直连探针（放大） | `max_tokens=4000`，~700 prompt tokens | **HTTP 200** |
| **流水线真实请求** | `max_tokens=4000`，**3134 prompt** + 最长 4000 completion | **HTTP 402** |

⇒ 我据此先写成"外部正常 ⇒ 是产品缺陷"，**方向是错的**。与全部观测一致的最简解释是
**余额仅够小额请求**（首次 e2e 的 `page_analysis` 成功一次 6339 tokens 后，
账户就再没撑住过大请求）。
📌 **可执行结论**：
① 探针要**复刻被测请求的形状**（端点 + 模型 + `max_tokens` + **提示词规模**），
   否则"探针 200"证明不了"产品请求会成功"；
② `402` 这类**额度失败**，报结论时**不得写成产品缺陷** —— 它是**额度**，不是逻辑；
③ ⚠️ **别把瞬时状态写成结构性结论**："余额为 0" ≠ "该账户不支持免费模型"
   （见 §四十二 ③ 的更正；该结论已证伪）。

### 二、驱动**不注入模型** ⇒ 静默落到默认档，失败被算到产品头上

产物级驱动原来只 POST `llm_provider` + `<prov>_api_key`，**从不设模型** ⇒ 沙箱
落到产品默认 `deepseek-ai/DeepSeek-V4-Pro`（**收费**档）⇒ 撞 402，而报告写着
"pipeline_terminal FAIL ⇒ 产品缺陷 优先查凭据是否发给了错误的提供方"。
⇒ **模型档位是验收的前提条件，不是可选优化**：
- 驱动**必须能把模型作为一等输入**（本项用 `PBC_E2E_MODEL`）；
- 且**必须读回生效值**再断言 —— 只断言"我 POST 了什么"没有判别力
  （新增护栏 `settings_llm_model_matches`：`GET /api/settings → providers[].model`）；
- **未指定模型时也要如实打印当前生效的模型名**（本轮正是靠这条才看出前两轮跑的是收费默认档）。

### 三、**别重复发明已有约定**（"同物不同名"是本项目的高频病）

项目里**早已有**模型覆盖约定：根 `e2e_run.py` 的 `resolve_llm_model` +
`PBC_E2E_MODEL` + `model_mismatch`（还有整整一个单测文件 `test_e2e_model_override.py`）。
我**没先查**就拟了新名 `PBC_E2E_LLM_MODEL` —— 一旦落地就是"同物不同名"：
运行方只会设一个，另一个**静默落到默认档**，正是上面第二条 402 误归因的翻版。
📌 **可执行结论**：**动手加环境变量/字段前，先 grep 一遍同名概念**
（`grep -rn 'PBC_E2E_MODEL' tests/ e2e_run.py`）。
修正后补**跨模块一致性护栏**：用 **AST 取 `os.environ.get(<字面量>)`**
（不用正则 —— 正则会被注释/别处同名 token 满足，§二十八盲区一），
断言两个 driver 读的是**同一个名字**，并配阳性对照（喂一个**故意不同**的合成源码，
必须能识别出来；找不到函数时必须**报错**而非静默返回空）。

### 四、证据链要**分层且逐层能证伪上一层**；层数不够就是没验

D4 最终的判据是**三层**，缺一层都不算数：

| 层 | 判据 | 它能证伪什么 |
|---|---|---|
| ① 驱动自述 | `E2E_RC=0` / `Total: 26 passed` | **什么也不能**（第一次就是这层绿而全盘是假绿） |
| ② 产品权威端点 | `GET /api/jobs/{id}/llm_audit` ⇒ `success=3/3` | 证伪①："终态成功"**不蕴含** LLM 被调用过 |
| ③ 直查库 | `data.db`：终态 + `llm_call_audit` + `findings.source` 分布 | 证伪②：端点可能只报"有成功记录"而**模型被静默替换**（`model` 列才是真相） |
| ④ 抽样核对语义 | 拿夹具**地面真值**比 findings 描述 | 证伪③：跑通了**不代表判得对**（本项目 2026-09-17 有过"链路可用但模型不符"事故） |

📌 **推广**：**"验完"的定义 = 每一层都有独立证伪力，且都过了**。
只有① ⇒ 假绿；只有①② ⇒ 可能模型被替换；只有①②③ ⇒ 可能判得不对。
**报告里必须写明验到第几层**，不能说"端到端通过"就了事。

---

## 四十四、"恒假探针"与"从未被执行的判定"两连坑（Round 59 W2 实跑，2026-09-23 实测）

### A. 🔴 API 返回值可以**恒假**：`SHFileOperationW` 删除成功时也返回 2

**现场**：`clean_dist.py --apply` 打印
`[apply] dist-electron: FAIL — SHFileOperationW rc=2 (0x00000002)`，
而**磁盘上那两个残壳已经消失**（`ls -d dist*` 只剩 `dist` + 一个完整产物）。

**隔离实验**（不猜，三组各自可判）：

| 组 | 做法 | 结果 | 排除了什么 |
|---|---|---|---|
| A | 对**刚建的小目录**调用 `to_recycle_bin` | `ok=False` 但**目录也没了** | 与"文件被占用"**无关** |
| B | husk 目录重命名探测 | 目录已不存在（A 已给答案） | —— |
| C | husk 内最长路径长度 | 同上 | —— |

⇒ 本机 `SHFileOperationW` 在**删除成功时仍返回 `rc=2`**
（`ERROR_FILE_NOT_FOUND`）。旧实现 `if rc != 0: return False` 因此**恒假**。

⚠️ **假失败比假成功更危险**：操作者（或自动化）会把"已经删掉了"读成"没删掉"，
进而升级成 `rm -rf` —— 而本项目**明令禁止**该操作。报错看起来"保守/安全"，
实际是在把人推向更危险的动作。

**修**：判据换成**可观测事实**（删前存在 ⇒ 删后必须不存在）；
`rc` / `fAnyOperationsAborted` **只降级为说明文字**，不再参与判定。

📌 **推广**：判据要落在**"我们真正想要的结果"**上，而不是某个 API 的返回码上。
§二十六 已说"恒真与恒假一样零判别力" —— 本轮的补充是：
**恒假不需要"东西坏掉"，只需要一个 API 怪癖**，而且它常披着"保守"的外衣。

### B. 🔴 被测对象被 stub 掉 ⇒ 那段判定逻辑**从未真实执行过**

`tests/unit/test_clean_dist.py` 里所有涉及回收站的用例一律
`monkeypatch.setattr(cd, "to_recycle_bin", …)` —— 判定分支被**整个替换**，
实际覆盖率为**零**。所以"46 个用例全绿"与"判定逻辑对不对"**毫无关系**。

**补法**：新护栏**只替换 Win32 调用**（`shell32.SHFileOperationW`），
让 `to_recycle_bin` **真实执行**；并配**成对反例**（`rc=0` 但路径仍在 ⇒ 必须 FAIL）
—— 没有这一条，"恒 `True`"也能让正例通过（判据无判别力）。

**变异验证 4/4 CAUGHT**（`devlogs/_verify/mutate_clean_dist.py`）：
`M1` 回退成 rc 判据｜`M2` 恒 `True`｜`M3` 丢掉中止诊断｜`M4` "路径不存在"也报成功。

📌 **推广**：**"这个函数被测过"要看它有没有被真实调用**。
stub 掉边界依赖是对的（单测不能真删文件），但**必须至少有一个用例走真实实现**；
否则你测的是 stub，而**判据的判别力不能由 stub 提供**。

### C. 🔴 包管理器操作被打断 ⇒ 声明没写、树回滚，而你看到的是"已经升过了"

**现场**：`npm install --save-dev electron-builder@26.15.3` 被 SIGTERM 打断。
此后 `node_modules/electron-builder/package.json` 读到 **26.15.3**（打断时的残留）
⇒ 据此判定"已升级"。但 `package.json` 里的**声明仍是 `^25.1.8`**，
于是随后用于收敛的 `npm install` 按声明把树**装回了 25.1.8**，
`npm audit` 的 16 条 advisory **原样重现**。

**判据要取两处**：`package.json` 的**声明** + `package-lock.json` 的**解析**；
**不要**用 `node_modules/<pkg>/package.json` 当"装了什么"的证据 ——
它可以是中断残留。

📌 **推广**：**"变更已生效"的判据 = 声明 + 锁 + 实际安装三者一致**。
单独看任一处都会被中间态骗到，而**被打断的包管理器操作会留下"看起来正确"的树**。
（同源记忆：本机 `npm install`/`pip install` 在无 TTY 下会撞 safe-delete 批量守卫
而静默卡死 ⇒ 一律后台跑 + `CODEBUDDY_SAFE_DELETE_BULK_THRESHOLD` + `< /dev/null`。）

---

## 四十五、"拿不到的版本"与"够不着的下载源"：依赖升级的两道环境关卡（Round 59 W2，2026-09-23 实测）

### A. 🔴 依赖**包**存在 ≠ 依赖**二进制**可得

- `npm view electron@43.7.5 version` ⇒ **43.7.5**（npm 包**在**）。
- 但打包/运行要的是 `electron-v43.7.5-win32-x64.zip`，而它在
  **npmmirror / 华为云 / 腾讯云 / 清华 四家全部 404**；
  **43.7.4 在四家全部 200**（互为阳性对照 —— 证明探针能认出"有的"）。
- ⇒ `electron` 必须按**精确版本**钉 `43.7.4`。写 `^43.7.4` 会解析回 43.7.5，
  **又一次拿不到**，而且**看起来一切正常**，直到打包那一步才炸。

📌 **推广**：升级依赖时要验的是**"那个能用的东西在不在"**，不是"版本号存不存在"。
**npm registry 的版本列表** 与 **发布物的下载源** 是两件事。
同类要单独探并各自准备阳性对照：PyPI 的 wheel、Docker 镜像 tag、模型权重。

### B. 🔴 "网络正常"是**分域**的：GitHub 断 ≠ npm 断

同刻实测（2026-09-23）：

| 目标 | 结果 |
|---|---|
| `registry.npmjs.org` | ✅ 可达（`npm install` 455 包成功） |
| `github.com` 直连 | ❌ `000` |
| `github.com` 走代理 | ❌ `502 Bad Gateway` |
| `npmmirror.com/mirrors/electron/` | ✅ 可达（zip 可取，206） |

两个误导源：
1. **环境里没有代理变量** ⇒ 只看到 electron-builder 抛 `502` 时，第一反应会去查
   "代理是不是配错了"，实际是"**GitHub 这条路根本不通**"。
2. `git` **自己**配了 `http.proxy=127.0.0.1:7897`（该端口**未监听**）⇒
   `git ls-remote` 报 `Failed to connect to 127.0.0.1 port 7897`。这与"electron 下不动"
   **是同一条不通的路的两种表现**，别当成两个独立问题去修。

📌 **推广**：**"网络通不通"必须按域名分别判**，且用**语义化探针**
（`git ls-remote` 返回一个 sha 或一个错误；`curl -w %{http_code}` 会骗人）。
另注意**工具自带的代理配置**（git / npm / pip 各有一套），它可能与 shell 环境变量**不一致**。

### C. 修法要落在**可复现的构建入口**里，不是"这次手工加个变量"

- 首次冒烟失败后，手工 `ELECTRON_MIRROR=… electron-builder` 是能过的；
  但那样**构建不可复现**（下次、换机器必再踩），且失败现象一样隐蔽。
- 已固化进 `build.ps1` **Step 3.0**，并**不覆盖**外部已设的值
  （自建镜像 / 企业代理仍可接管）。

📌 **推广**：环境依赖的"绕法"必须**固化进构建脚本**并注释清判据与出处；
**手工一次性绕行 = 把坑留给下一个人**。同时要保留"不覆盖外部配置"的口子 ——
否则等于把本机的网络拓扑写死进项目。

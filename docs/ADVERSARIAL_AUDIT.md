...
| # | 级 | 位置 | 发现 | 证据类型 |
|---|---|---|---|---|
| 1 | **P1** | `llm_finding_guard._check_declared_order` | 方向反 ⇒ **整条抑制**，丢掉"纠正方向后依然异常"的真线索 | 回放 + 直查库 |
| 2 | P2 | `llm_finding_guard.apply_review` | **死代码**，且与两个调用方三份重复实现（已漂移） | 回放 + 读码 |
| 3 | P2 | `llm_finding_guard._check_grounding` | 把"当前年份 2026"当幻觉数字 ⇒ 降级**理由错误**（会误导排障） | 回放 |
| 4 | P2 | 布尔/勾选类处置 | 同类问题两种处置（p39/p46 抑制 vs p45 降级），且 p45/p46 **未视觉核实** | 回放 |
| 5 | P3 | `llm_finding_guard._layer_of` | 靠**文案子串**反推复核层级 ⇒ 改一个字即静默落 `L?` | 静态 |
| 6 | P3 | `_check_declared_order` | 只扫 `description`，不扫 `ocr_text`（与其它检查不一致） | 静态 |

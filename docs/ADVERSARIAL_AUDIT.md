...
| 2 | **运行时看门狗**（§2 的 P0） | **兜底一半已完成**（2026-09-16：`run_cpu` 超时 + 池回收，属缺陷修复，已随 `b1c48bc` 落地）。**剩下的一半是"周期巡检"**（`jobs.last_activity_at` schema v11→v12 + 状态机写入点 + 后台扫描 + 阈值分级），它触及状态机与写库关键路径 → 想先确认要不要做、以及阈值怎么定（见 `docs/RUNTIME_WATCHDOG.md`） |

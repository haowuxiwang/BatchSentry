...
| 页 | 文案 | 提取值 | 处置 |
|---|---|---|---|
| p39 | …参数值为否，不符合规格范围 | `否` | **抑制** |
| p46 | 清洗过程是否无异常参数值为否，超出规格范围 | `否` | **抑制** |
| p45 | 步骤 2 中是否将柱内甲醇压干 勾选了否，不符合操作指导 | `None` | **仅降级** |

**根因**：`_NOT_A_VALUE` 词表含"不符合操作指导" ⇒ `_declared_spec_value` 返 `None`
⇒ fail-open ⇒ 只降级。即**同一件事（勾选了否）的处置，取决于措辞有没有撞上词表**。

⚠️ 且 **p45/p46 从未做视觉核实**（Round 43 只核实过 p39，结论是"是/否框**全部勾的是「是」**"
⇒ LLM 读反）。在这三页里，`否` 究竟是不是真"否"，**目前无证据**。

...
> ⚠️ **口径更正（B4-5，2026-09-21）**：上表的 **477 含 36 条 `chapters`
> 章节标题元数据**（无正文、无 `entry_id`、检索器不索引）⇒ **可检索语料真值 = 441 条**。
> 根因是 `count_kb_entries` 把 `chapters` 一并累加，门禁指标因此虚高 8%。
> 现已修正为**只计 `entries`**，且门禁 `kb_corpus` 同时报出两个数字；
> 护栏 `test_gate_count_equals_retriever_count` 把门禁口径与 `store.entries()` 绑定。
...
# 16. 第四轮对抗性审查（2026-09-20，Round 46）

> **审查对象**：**工作树领先于产物的两项未发布改动** ——
> **B1-6**（`core/rules/llm_finding_guard.py`，LLM 收权复核）与
> **B1-9**（`core/rules/rule_time.py`，时序判据分级）。
> **基线**：产物 `v1.1.9`（`1379f8e`）+ 工作树 `f724c3b`（`git status` 干净）。
> 与前三轮不同，**本轮审的是"尚未分发的改动"**，目的是在冻结 v1.2.0 之前把
> 新引入的缺陷挡在产物之外。

## 16.0 一页结论

- ✅ **B1-6 / B1-9 的"方向"是对的**。LLM 假阳性五类（日期方向反 / 列串位 /
  跨字段串位 / 类型错配 / 凭空数字）确实**根因同一个**（LLM 既从 OCR 挑值、又自己下
  判定），把判定收回规则层对症；B1-9 纠正「区间重叠 ≠ 倒序」亦经真实数据确证无误。
- 🔴 **但 B1-6 引入了一处新的漏检面（P1）**：`llm_finding_guard._check_declared_order`
  把"方向词写反"当成"**整条结论不成立**"而**抑制**。当被比较的两者**都来自记录内部**
  时，被抑制掉的条目里**有真线索** —— 实测 p38 两条
  「签名时间 2027.01.17 早于 生产日期 2025年01月20日」抑制后，
  **该页再无任何条目提及「2027」这个未来日期，且规则层无兜底**。
  这与 B1-6 自己写下的原则（「只对**逻辑不成立**的用抑制」）冲突：
  p38 的底层事实**是成立的**，不成立的只是 LLM 的**方向词**。
- ⚠️ **分发判定：本轮不建议冻结 v1.2.0** —— 见 **16.4**（环境阻塞 + 质量门禁两条独立理由）。
- 📌 **可维护性副产物**：收权模块自身存在**死代码 + 三份重复实现**（16.2 #2），
  正是用户问的"是否足够模块化 / 职责单一"在**本轮新增代码上**的答案：**部分不合格**。
  ✅ **已闭环（Round 47，见 16.6）**：`apply_review` 已接线为**写路径唯一入口**，
  并借一次全仓扫描把同类"生产死接口"一次性清零（**7 → 0**）。
...
# ① 从 Round 42 的 51 页真实隔离库回放（必须拷 db + wal + shm 三件套，缺 WAL 读不到新数据）
cp "$TEMP/pbc_e2e_appdata/PBC/data.db"{,-wal,-shm} devlogs/_replay/
"$PY" devlogs/_replay/replay.py        # 基线：抑制 26 / 降级 42 / critical 49 → 0
"$PY" devlogs/_replay/probe_audit.py   # 本轮新增：专项质疑三条可疑路径

# ② 直查隔离库，核对"被抑制的线索有无兜底"
#    job = 6f80145a（Round 42 real 轮）
```

对本轮**每一条断言**都要求下面两种证据之一：**真实数据回放**或**直查隔离库**。
凡只能靠读码推断的，一律标 **P3** 并注明"静态结论"。

## 16.2 本轮新发现

| # | 级 | 位置 | 发现 | 证据类型 | 状态 |
|---|---|---|---|---|---|
| 1 | **P1** | `llm_finding_guard._check_declared_order` | 方向反 ⇒ **整条抑制**，丢掉"纠正方向后依然异常"的真线索 | 回放 + 直查库 | ✅ **当日已修**（B1-11，见 16.5） |
| 2 | P2 | `llm_finding_guard.apply_review` | **死代码**，且与两个调用方三份重复实现（已漂移） | 回放 + 读码 | ✅ **Round 47 已修**（B3-4，见 **16.6**） |
| 3 | P2 | `llm_finding_guard._check_grounding` | 把"当前年份 2026"当幻觉数字 ⇒ 降级**理由错误**（会误导排障） | 回放 | ⏳ 未修 → **B2-12** |
| 4 | P2 | 布尔/勾选类处置 | 同类问题两种处置（p39/p46 抑制 vs p45 降级），且 p45/p46 **未视觉核实** | 回放 | ⏳ 未修 → **B1-12** |
| 5 | P3 | `llm_finding_guard._layer_of` | 靠**文案子串**反推复核层级 ⇒ 改一个字即静默落 `L?` | 静态 | ⏳ 未修 |
| 6 | P3 | `_check_declared_order` | 只扫 `description`，不扫 `ocr_text`（与其它检查不一致） | 静态 | ⏳ 未修 |

### #1（P1）方向反被当作"整条不成立"⇒ 抑制真线索

`probe_audit.py`【1】列出**全部 4 条**由 `_check_declared_order` 抑制的条目：

| 页 | 左日期 | 右日期 | 跨度 | 参照物 | 该不该抑制 |
|---|---|---|---|---|---|
| p2 | 2025.01.30 | 2026.09.18 | 1.63 年 | **当前日期** | ✅ 正确（"过去 vs 现在"本是常态，比较本身无意义 = B1-4 型） |
| p27 | 2025.01.29 | 2015.01.25 | **10.01 年** | 记录内日期 | ❌ 年份误读线索被抑制 |
| p38 ×2 | 2027.01.17 | 2025.01.20 | 1.99 年 | 生产日期 | ❌ **未来日期**线索被抑制 |

**兜底核查（直查隔离库 `6f80145a`）**：

- **p27**：同页另有 `[llm_page] year_contradiction 取样日期 2025.01.29 与生产日期 2015.01.25 年份矛盾`
  **未被抑制** ⇒ 线索**碰巧**保住。⚠️ **这是巧合而非设计** —— 若 LLM 只报了
  `signature_time_anomaly` 一条，线索就没了。
- **p38**：抑制后该页只剩 `[rule] self_review`（操作人与复核人同为「周新」）；
  两条 `signature_time_anomaly` 与两条 `suspicious_date` **全部被抑制**
  ⇒ **「2027」这个未来日期在结果里完全消失**。
  规则层 `_check_signature_time_anomaly` **未**独立报出该页（全 job 的 `source='rule'`
  条目里没有它）⇒ **无兜底**。

**判据为何过强**：`_check_declared_order` 的结论是"**方向词**与事实相反"，
但代码把它**升级**成了"**整条 finding** 不成立"。二者不等价 ——
p38 的 `2027` 比生产日期晚 2 年**是事实**，且 `2027` 是**未来日期**（当前 2026-09），
这本身就是 GMP 里必然要追问的异常；被抑制等于**把异常连同错误的表述一起丢掉**。

**修复方向**（16.5 落地）：区分**参照物** ——
- 参照物是**"当前日期/当前年份"** ⇒ 该比较无意义 ⇒ **抑制**（保留 B1-6 的收益）；
- 参照物是**记录内另一日期** ⇒ 方向词错但日期对可能真有跨度问题 ⇒ **降级保留**，
  并在 description 里**纠正方向表述**（让复核者看到"应为早于/晚于"）。

判据不引入墙钟、不引入新阈值常量 —— 只用文案里是否出现"当前日期/当前年份"，
既避开 **B1-7**（墙钟基准致 finding 不可复现），也避开"两份阈值表漂移"这一本项目高发坑。

### #2（P2）`apply_review` 是死代码，且三份实现已漂移

```
$ grep -rn "apply_review" --include=*.py .
./core/rules/llm_finding_guard.py:465:def apply_review(          ← 定义
./tests/unit/test_llm_finding_guard.py:13/239/242/256            ← 仅测试引用
```

**零生产消费点**。而两个真正的调用方各自手写了同一段"重建 + 追加降级理由"逻辑：

- `core/pipeline/stage2.py:390-396`
- `core/pipeline/stage3.py:92-96`

三份实现的差别：`apply_review:484` 对 description 做了 `.rstrip()`，两处调用方**没有**
⇒ **已经漂移**。这正是"同一段逻辑抄多份，迟早不一致"的同型坑（项目在
`_get_analyzed_pages`、`is_congestion_error` 上各踩过一次）。

### #3（P2）`_check_grounding` 把"当前年份"当幻觉 ⇒ 降级理由错误

`probe_audit.py`【2】19 条 grounding 降级里，**14 条**是 `suspicious_date` 的
**"当前年份 2026"**，例如：

```
p38  [warning→info] suspicious_date  数字=['2025','2026','2025']
     原文: 生产日期 2025年01月20日 与 当前年份 2026年09月18日 不符
```

**"2026"由系统时钟得来，本就不该出现在 OCR 原文里** ⇒ 判"疑似提取幻觉"**理由错误**。
（降级的**方向**是对的 —— 这些条目确实不构成异常 —— 但**理由**错误会误导排障，
属与 **B2-6**（`measurements=` 恒 0）同一类"日志/理由说谎"的问题。）

### #4（P2）布尔/勾选类处置不一致 —— ✅ Round 53 已修（含**根因更正**）

`probe_audit.py`【3】：

| 页 | 文案 | 提取值 | 旧处置 | 原页真值（Round 53 核验） |
|---|---|---|---|---|
| p39 | …参数值为否，不符合规格范围 | `否` | **抑制** | **`√ 是 / □ 否`** |
| p46 | 清洗过程是否无异常参数值为否，超出规格范围 | `否` | **抑制** | **`√ 是 / □ 否`** |
| p45 | 步骤 2 中是否将柱内甲醇压干 勾选了否，不符合操作指导 | `None` | **仅降级** | **`√ 是 / □ 否`** |

**根因（⚠️ 本轮更正，原记载不准）**：~~`_NOT_A_VALUE` 词表含"不符合操作指导"
⇒ `_declared_spec_value` 返 `None`~~ —— **不成立**。`_NOT_A_VALUE` 是拿**取到的值**
去比；而 p45 的文案里**根本没有 `值` 字**，`_BARE_VALUE_RE`（只认「…值为 X」/引号）
**压根匹配不上** ⇒ 取不到值 ⇒ fail-open。真因是**取值器只认一种措辞**：
处置档位由**动词**（`值为` vs `勾选了`）决定，与问题性质无关。

**原页核验结论（渲染 p39/p45/p46）**：三页的「是」框才是有 √ 的那个 ⇒ 三处"否"主张
**全是 LLM 读反**（假阳性），本就该得到**同一种**处置。

**修法**：`core/rules/llm_finding_guard.py` 新增**统一类型闸门**
`_declared_boolean_literal` + `_BOOLEAN_CLAIM_RE` —— 判据是"文案断言了勾选**结果**"
这一**性质**，与用哪个动词无关；且在 `_check_spec_value_shape` 里**先于"取值"判定**。
负向预查 `(?![一-龥])` 排除疑问词「是否」本身（否则「记录中勾选是否正确」这类
**评价勾选动作**的文案会被误判）。文案抽成单一实现点
`_boolean_type_mismatch_reason`（`_layer_of` 靠「勾选/布尔形态」子串归层 ⇒ 是**契约**）。
**实测**：三条 ⇒ kept=0 / downgraded=0 / **suppressed=3**，层号全 `L1-value-shape`、
理由逐字相同。**变异验证 5/5 CAUGHT**（`devlogs/_verify/mutate_b112.py`）。

➡ **未覆盖的相邻面**：**规则层** R8 `_check_check_consistency` 也在同三页报了
「勾选为“否”」warning（8 条，同为假阳性）—— 它读 `checks[].selected`，同样不可靠。
其中 `selected='否'` + `marker='☐'`（**空框**）**可从记录内部证明自相矛盾**（可确定性
修复），`selected='否'` + `marker='√'` 则只有原图能证伪。⇒ 见 `docs/TODO.md` **B1-15**。

### #5 / #6（P3，静态结论）

- **`_layer_of`（:454）靠文案子串反推层级**：`"勾选/布尔形态" in reason` 之类。
  改一个字（"勾选/布尔" → "布尔"）就**静默落到 `L?`**，而审计 evidence 里记的正是这个值。
  正解是让各层返回 `(layer, reason)` 元组，而非事后嗅探字符串 ——
  与项目既有纪律「**断言解析后的结构，永不断言序列化后的字符**」同源。
- **`_check_declared_order` 只扫 `description`**（`:295`），而 `_check_grounding`、
  `_check_speculative` 都用 `_finding_text`（description + ocr_text）。
  万一 LLM 把方向断言写进 `ocr_text`，该检查即失效（方向保守 = fail-open，不算漏检）。

## 16.3 被排除的假设（对抗审查的一半价值在这里）

以下两条**曾被列为可疑，经实测后排除** —— 记录在此，避免下一轮重复怀疑：

1. ~~**stage3 收权上下文为空 ⇒ L1/L2/L3 静默失效**~~ —— **不成立**。
   `_struct_by_page` / `_raw_by_page` 在 `stage3.py:52-57` **无条件填充**
   （只要 `json.loads` 成功，含 `_parse_error` 页也会入字典）；`json` 坏页不在字典里
   ⇒ `.get()` 返 `None` ⇒ fail-open。收权**不会**因上下文缺失而空转。
2. ~~**p9「T2101a~d 压力值超出规格范围」被误降级**~~ —— **不成立**。
   精确复现显示其降级理由是 **L4 severity 封顶**（LLM 独断 critical ⇒ warning），
   属**设计内正常行为**；且 `_NOT_A_VALUE` 让提取器返 `None` ⇒ **没有**被 L1 误杀
   ⇒ **B1-6 针对 p9 的修复确实生效**。

## 16.4 分发判定：本轮**不**建议冻结 v1.2.0

两条**相互独立**的阻塞，任一条单独成立都不应打包：

**(a) 环境阻塞（硬）**：`app.asar` 正被**宿主**持有 —— Restart Manager 实测返回
`WorkBuddy(pid=24732)`：

```
$ python -c "...clean_dist.describe_holders('dist-electron/win-unpacked/resources/app.asar')"
WorkBuddy(pid=24732, Unknown)
```

`build.ps1:265-297` 在目标被占时**自愈到时间戳目录** `dist-electron-out-<stamp>`
⇒ 直接违反既有的硬要求「**仓库根只能有一个 `dist-electron*`**」。
（`clean_dist.py` dry-run 当前为 `待清理:(无)`、`dist-electron/` 完整 1.1.9 ⇒ **现状是干净的**，
不该为了打包把它破坏掉。）**解锁 = 完全退出宿主** —— 宿主内无法杀掉它自己。
⇒ **打包必须在宿主完全退出后的普通终端进行**，本会话内做不到。

**(b) 质量门禁（软，但优先级更高）**：本轮发现 **P1**（16.2 #1）。
B1-6 **尚未发布**（工作树领先产物），**现在正是修它的窗口期**；若带着 P1 冻结 v1.2.0，
"收权"就从净收益变成**净漏检** —— 而 GMP 场景**漏检代价 > 误报代价**。

**顺带厘清的"打包前置条件"现状**（供解锁后按序执行）：

| 条件 | 状态 |
|---|---|
| 工作树干净 | ✅ `git status` 为空 |
| 产物目录唯一性 | ✅ `dist/` + `dist-electron/`（1.1.9 完整），无时间戳变体 |
| 版本真值 4 处 | ⚠️ 仍为 **1.1.9**，冻结 v1.2.0 时需统一改（注意 `package-lock.json` 另有 2 处属**依赖自带**，批量替换必误伤） |
| 全量测试 | 见 16.5 |
| 门禁 6 项 | 须在**产物重建之后**跑（`tests_coverage` 含分发一致性，升版未重建必红，**这是正确信号**） |
| 产物锁 | 🔴 **被宿主持有** ⇒ 阻塞 |

## 16.5 修复与验证（同日完成）

两条 P1 **都不是"实现写错了"，而是"判据把『判不了』当成了『判据确凿』"** ——
**同一个工程错误在两条 L3 路径上各犯一次**。修复遵循同一条原则：
**弱证据只降级，强证据才抑制**。

### 修复 1：`_check_declared_order` 按**参照物**分级

| 情形 | 旧行为 | 新行为 |
|---|---|---|
| 参照物含「当前日期 / 当前年份」 | 抑制 | **抑制**（不变 —— 该比较本身无意义，B1-4 型） |
| 两侧**都是记录内的日期** | 抑制 ❌ | **降级**（错的只是表述；日期对本身仍需人工核对） |

新增常量 `_CURRENT_REF_RE`，刻意**不引入墙钟**（避开 **B1-7**：墙钟基准 ⇒ finding
跨年份不可复现）、**不引入新阈值常量**（避开"两份阈值表迟早漂移"这一本项目高发坑）——
只用文案里是否出现"当前日期/当前年份"来分档。

### 修复 2：`_recompute_time_reversal` 改为**三态**（`bool` → `bool | None`）

| 该页结构化数据 | 旧行为 | 新行为 |
|---|---|---|
| 确有倒序 | 保留 | 保留 |
| 有可解析工序、且**均无**倒序 | 抑制 | **抑制**（不变 —— 这才是真正的强证据） |
| **一个可解析工序都没有**（字段空/占位/结构化缺失） | 抑制 ❌ | **降级**（判不了，fail-open） |

### 实测（51 页真实隔离库回放，job `6f80145a`）
...
| 降级 | 42 | **55** |
| 保留 | 88 | 88 |
| **critical（LLM 假阳性）** | **0** | **0**（保持不变） |

**修复后仍被抑制的 13 条，逐条都是强证据**（这是修复没有把闸门放垮的证据）：

| 页 | 条数 | 层级 |
|---|---|---|
| p2 | 2 | L3-declared-order（参照物 = 当前日期） |
| p6 | 2 | L3-time-reversal（该页有可解析样本且确无倒序） |
| p17 | 4 | L1-value-shape（`'A'` / `'2025.01.21'` 列串位） |
| p24 | 1 | L3-time-reversal |
| p39 | 2 | L1-value-shape（布尔值"否"塞进数值类型） |
| p46 | 2 | L1-value-shape（同上） |

被释放的 13 条里，**真线索**包括：p27（年份差 **10 年**）、p38 的 **2027 未来日期** ×2、
**p43 ×4 / p46 文案明写「开始时间晚于结束时间」×1**。

⚠️ **本次修复的代价（诚实记录）**：p38 的「清洗罐搅拌结束 18:30 早于开始 19:15」
（Round 43 已视觉核实为**跨行串位幻觉**）从"抑制"变为"降级" —— 因为该页 **0/3 可解析**，
规则层**确实判不了**，只能留给人工。这是"**漏检代价 > 误报代价**"原则下的**有意取舍**：
用几条 warning 噪声，换 p43/p46 的真倒序不被吞掉。

### 护栏与变异验证

- `tests/unit/test_llm_finding_guard.py`：20 → **23** 例。新增"p43 判不了 ⇒ 只降级"、
  "structured 缺失 ⇒ 只降级"、"p27 跨度 10 年 ⇒ 只降级"；并**改写 p38 原用例的期望**
  （抑制 → 降级）。⚠️ 改期望值**同时给出了理由**（写进 docstring），不是只改数字。
- **变异验证（实测，脚本 `devlogs/_replay/mutate_r46.py`）** —— 三个方向都必须变红：

  | 注入的变异 | 结果 |
  |---|---|
  | ① 三态退化：`return False if samples else None` → `return False` | ✅ **2 failed** / 118 passed |
  | ② 方向判据退化：参照物不分档，一律抑制 | ✅ **2 failed** / 118 passed |
  | ③ "当前日期"参照失灵（`_CURRENT_REF_RE` 永不命中） | ✅ **1 failed** / 119 passed |
  | 还原后复跑 | ✅ **120 passed**；`git status` 确认**无残留变异** |

- **全量回归**：**2771 passed / 0 failed**（基线 2767 + 本轮新增 4 例 = 自洽）。
  ⚠️ 全量跑首次被**沙箱的 bulk-delete guard** 中断（环境的 safe-delete shim，
  非产品缺陷）⇒ 按既有做法加 `CODEBUDDY_SAFE_DELETE_ENABLED=0` + `BULK_THRESHOLD=100000` 复跑通过。
- **静态检查**：`ruff check` 改动前后**同为 11 errors**（均为既有、且不在门禁范围内）
  ⇒ **本轮零新增 lint 问题**。
- **文档一致性**：顺带删掉了 `review_llm_findings` docstring 里承诺但**从未实现**的
  `_guard_corroborated` 标记 —— 文档不得承诺一个不存在的字段。

## 16.6 Round 47 处置：16.2 #2（B3-4）已闭环

> 用户指定优先处理 16.2 的 **#2**（"`apply_review` 是死代码 + 三份重复实现已漂移"）。
> 处置原则：**能统一到唯一实现点的就接线，纯负债才删** —— 不做"一刀切删死代码"。

**① 定位（先定位再动手）**：把上一轮那句"建议一并排查还有多少此类死接口"真正做了 ——
写全仓 AST 扫描 `devlogs/_lint/deadscan.py`（判据 = 定义在库代码里 + **生产侧零引用**），
一次问清：**7 个**（5 个「仅测试引用」+ 2 个「彻底无引用」）。
⚠️ 首版扫描器**太粗**（把 pytest fixture、FastAPI 路由处理器都算进来 ⇒ 误报 **268** 个）。
**教训**：盘点的判据必须与"当初判定 `apply_review` 为死代码"的**同口径**，
否则清单会被噪声淹没、失去可操作性。

**② 处置（7 项，逐项见 `docs/TODO.md` B3-4）**
- ✅ **接线**：`apply_review` ⇒ stage2/stage3 改用它（重建逻辑只剩一个实现点，漂移消除）。
- ✅ **统一**：`source_version` / `entries_by_source` / `is_canonical` 三个"函数是死的、
  但生产侧**内联了同一逻辑**" ⇒ 让**生产侧改用它们**。
  ⚠️ 关键判断：前两个原本是测试里的**独立判据（oracle）**（用各源版本**重算**组合版本、
  用各源并集**重算** `entries()`）⇒ **直接删会削弱测试判别力**，正解是让生产侧用它们。
- ❌ **删除**：`rule_by_type`（与 `rule_coverage` 的 `by_type` **语义不同**：含/不含禁用规则
  ⇒ **不可合并，只能删**）、`begin_children`（**冗余第二构造入口**）、`vote_report`
  （docstring 谎称"落 audit" ⇒ 它的存在本身在**误导**）。后者的缺口另登记 **B3-5**。

**③ 护栏（防再复制）**：`tests/unit/test_llm_guard_single_impl.py`（6 例）
- 正向：stage2/stage3 必须以 AST 形式调用 `apply_review`；
- 反向：`core/pipeline/` 不得直接调底层引擎 `review_llm_findings`（= 绕过唯一入口）；
- 反向：物化降级的标记 token `to_severity` 不得出现在 `core/pipeline/`；
- **正向对照**：该 token 必须确实存在于唯一实现点 —— 否则反向断言是**空断言**。
- ⚠️ 断言用 `ast.Constant` **精确等值**而非文本搜索：pipeline 里 `｜` 另有合法用途（错误文案），
  文本搜索会**假红**。

**④ 变异验证**（`devlogs/_lint/mutate_b34.py`，4 个方向全部符合预期）

| 变异 | 期望 | 实测 |
|---|---|---|
| M1 把重建逻辑抄回 stage2 | 红 | ✅ **3 failed**（(a)(b)(c) 三条） |
| M1 还原 | 绿 | ✅ 6 passed |
| M2 改掉标记 token（模拟"扫不到"） | **正向对照**红 | ✅ **1 failed**（证明 (c) 非空断言） |
| M2 还原 | 绿 | ✅ 6 passed + **逐字节还原自校验通过** |

⚠️ **变异脚本首版自伤**（已修，记入 `PROJECT_PITFALLS.md` **§二十四**）：用
`read_text(utf-8-sig)` + `write_text` 改源码，**静默**给两个无 BOM 的文件**加了 BOM**
（BOM 是**内容**、会进提交），并把 CRLF 折成 LF。改**字节级**读写 + `finally` 自校验，
再用 `cmp` **独立复核**（不采信脚本自述）。

**⑤ 验收**：全量 `tests/unit + tests/integration` = **2776 passed / 0 failed**
（基线 2771 + 新增护栏 6 − 删除的直接单测 1 = 自洽）；`deadscan` 重跑 =
**生产死接口 0 + 仅测试引用 0**。

**⑥ 对用户四个元问题的更新回答**：B3-4 闭环后，"**是否足够模块化 / 职责单一**"这一项
在**本轮涉及的范围**内已由**部分不合格**转为**合格**（写路径单一实现点 + 全仓死接口为零）；
但 **B3-1 / B3-2 / B3-3 / B3-5 仍在**，故整体结论**仍是"部分"**，不夸大。

### 16.6.1 打包可行性（本轮再次实测，结论未变）

用户要求"直至满足打包要求 → 重新打包 → 对产物做端到端测试"。**本轮仍不可打包**，
且这次用**直接探针**（而非推断）确认：

```
clean_dist.who_holds(dist-electron/win-unpacked/resources/app.asar)
  -> [{'pid': 24732, 'app': 'WorkBuddy', 'type': 'Unknown'}]      # 宿主进程
clean_dist.locked_files(dist-electron)
  -> ['win-unpacked/resources/app.asar  (PermissionError: errno=13 winerror=32)']
```

`winerror=32` = `ERROR_SHARING_VIOLATION` ⇒ **该文件不可改名** ⇒ 含它的目录亦不可改名
⇒ electron-builder 无法替换 `win-unpacked`，只能**自愈到时间戳目录**，直接违反
"仓库根只能有一个 `dist-electron*`"的硬要求。
**解锁唯一方式 = 完全退出宿主进程**（我在宿主内杀不掉它自己；加杀软白名单无效）。
⇒ **不在本会话内升版**：升版必须紧跟重建，否则 `release_gate` 的 `tests_coverage`
（含分发一致性）会红 —— 那会留下一个"门禁红"的工作树，比不升版更糟。

---

# 17. 第五轮对抗性审查（Round 58，2026-09-23）

> 基线：`e9d9e25`（v1.2.0 产物已建，未冻结）。
> 本轮**换了三个此前从未审过的维度**：① **依赖与运行时的新鲜度**（漏洞库 + 框架
> 生命周期）；② **守卫的时序位置**（保护的是"效果"还是"解析器"）；③ **产物的资源足迹**
> （磁盘/内存/延迟的实测分布）。
> 与前四轮的差别：前四轮反复审的是**产品逻辑**（规则、LLM、降级、状态机），
> 这一轮审的是**产品所依赖的东西**，以及**守卫放在哪个位置**。
> 复现命令见 §17.9。所有数字均为本轮实测，非推断。

## 17.0 一页结论

**判定：本轮不建议分发。** 原因不是产品逻辑退化（§17.2 实测性能与功能面均正常），
而是**两条从未被任何门禁覆盖的供应链风险**（§17.3）：

| 编号 | 严重度 | 一句话 |
|---|---|---|
| **S1** | **P1** | 本地守卫在**体解析之后**才执行 ⇒ 任意网页可跨站喂畸形 multipart，实测 **0.53–10.26 s/请求** 的 CPU 消耗，且请求**根本没走到守卫**（返回 422 而非 403） |
| **S2** | **P1** | `python-multipart 0.0.21`（解析每一次上传）有 **7 条 CVE**、`Pillow 10.1.0` 有 **17 条 CVE**（含 1 条任意代码执行、多条堆越界写）——两者**都直接处理不可信输入** |
| **S3** | **P2** | 产物内 **Electron 33.4.11 / Chromium 130.0.6723.191 / Node 20.18.3** —— Electron 33 于 **2025-04-29 EOL**，已停安全支持 **17 个月** |

**好消息**（必须一并说，否则结论失衡）：Electron 的进程隔离配置是**正确**的
（§17.3.4）；SSRF / 路径穿越 / 签名 URL 泄露 / 上传伪装这几条**已有一等对策**；
性能面**没有日常问题**（正常上传 54–104 MB/s，读端点中位 2–7 ms，§17.2）。

**未复核 / 诚实留白**见 §17.8 —— 尤其：本轮**没有**验证 S2 中每一条 CVE 对本应用
的**可达性**（只验证了依赖版本落后与两个包的**输入面位置**）。
...
"$PY" devlogs/_verify/osv_detail.py          # 取 CVE 号 / 修复版本 / 摘要
# ② 运行时身份（从产物二进制里读，不采信 package.json 的 ^ 范围）
"$PY" -c "…正则提取 Chrome/ Electron/ node.js/ 版本…"
# ③ 守卫时序：解析 vs 守卫，三组对照
"$PY" devlogs/_verify/probe_parse_before_guard.py     # ①②③: 404 / Host=evil / Host=本机
"$PY" devlogs/_verify/probe_browser_scenario.py       # 浏览器真实形态: Origin=evil + Host 合法
# ④ 正常上传 vs 畸形解析的吞吐对照（区分"性能问题"与"安全面"）
"$PY" devlogs/_verify/probe_upload_throughput.py
# ⑤ 资源足迹（内核 API 取 RSS，不解析 tasklist 的本地化文本）
"$PY" devlogs/_verify/probe_perf_audit.py
```

## 17.2 性能：**没有日常问题**（实测分布）

**产物**：`dist-electron-out-20260921-160111/win-unpacked`（384 MB，其中内嵌后端 109 MB）。

### 延迟（内嵌后端，12 次采样）

| 端点 | 中位 | p95 | 最大 |
|---|---|---|---|
| `/health` | 2.0 ms | 2.9 ms | 2.9 ms |
| `/`（首页 HTML） | 6.9 ms | 26.2 ms | 26.2 ms |
| `/api/jobs` | 5.6 ms | 7.6 ms | 7.6 ms |
| `/api/settings` | 4.0 ms | 5.0 ms | 5.0 ms |
| `/api/health/watchdog` | 2.9 ms | 3.8 ms | 3.8 ms |

并发 16×`GET /api/jobs`：墙钟 **62 ms**，单请求中位 48 ms，最大 54 ms —— 无锁争用迹象。

### 启动与足迹

| 指标 | 实测 |
|---|---|
| 后端冷启动 → `/health` 200 | **2.85 s** |
| 完整应用冷启动 → `/health` 200 | **5.53 s** |
| 主窗口就绪（D5） | +1.01 s（5 次连跑恒定）|
| 后端单进程 RSS | **142 MB**（压测后 144 MB，无泄漏迹象）|
| **完整应用整棵进程树 RSS** | **699 MB**（主进程 149 + 4 个 Chromium 子进程 406 + 后端 132 + conhost 12）|

⚠️ 699 MB 是**峰值常驻**。GMP 工位通常 8–16 GB，可接受，但应写进部署要求
（`DEPLOYMENT.md` 目前只有磁盘容量，没有内存下限）。

### 上传吞吐：**正常与畸形差 40–60 倍**（这是关键判别）

| 形态 | 1 MB | 4 MB | 8 MB | 16 MB | 32 MB |
|---|---|---|---|---|---|
| **正常 multipart（合法 PDF）** | 54 MB/s | 75 | 97 | 96 | **104 MB/s** |
| **畸形 part 头**（无 CRLFCRLF 终结） | 2.3 MB/s | 2.3 | 1.7 | — | — |

⇒ **日常上传不是性能问题**（32 MB 仅 0.31 s）。慢的只有畸形输入，
所以它属于**安全面**（§17.3 S1），不是性能面 —— 这条区分必须写清楚，
否则会把"攻击面"误报成"性能缺陷"，或反过来把攻击面轻轻放过。

## 17.3 安全

### 17.3.1 S1（P1，**本轮新发现**）：本地守卫保护的是"效果"，不是"解析器"

**既有实现**（`api/jobs/upload.py:54-60`）已经意识到跨站风险，注释写得很准：
> 上传端点是 multipart/form-data（CORS safelisted，浏览器不触发 preflight），
> 此前无守卫 — 恶意网页可跨站 POST 任意 PDF

对策是在**端点函数体第一行**加 `is_local_request(request)` 守卫。**问题在于位置**：
FastAPI 是在**调用端点之前**就解析请求体（端点签名要求 `UploadFile`）。

**实测（浏览器真实形态：`Host` 由 URL 决定必然合法，跨站身份在 `Origin`）**：

| 形态 | 1 MB | 4 MB | 8 MB | 16 MB | 状态 |
|---|---|---|---|---|---|
| A 格式正确 + `Origin: evil` | — | — | — | — | **403**（0.01 s）✅ 守卫有效 |
| B 畸形体 + `Origin: evil` | **0.53 s** | **1.82 s** | **4.19 s** | **10.26 s** | **422** ❗ |
| C 负对照：同样大体 → 不存在路径 | 0.01 s | — | 0.06 s | 0.10 s | 404 |

三条结论，缺一不可：

1. **B 的状态是 422 而不是 403** ⇒ 请求因**请求体校验失败**被返回，
   `is_local_request` **一行都没执行** —— 守卫不是"晚一点"，是"完全没被走到"。
2. **C 证明耗时来自服务端解析**，不是传输：同样 16 MB，不解析只需 0.10 s。
3. **A 证明守卫在"效果"上是有效的** —— 格式正确的跨站请求确实被 403 挡住，
   没有启动流水线、没有消耗用户配额。**已有的加固没有白做**，缺的是把闸门前移。

**影响面**：服务是**单 worker**（`server.py` 的 `uvicorn.Config(..., workers=1)`），
解析期间事件循环被占 ⇒ **UI 无响应**。触发方式 = 用户访问任意网页
（`multipart/form-data` 属 CORS safelist，无 preflight；`fetch(..., {mode:'no-cors'})`
或一个隐藏 `<form enctype="multipart/form-data">` 即可）。请求可重复、体积无上限。

**定级为 P1 而非 P0 的理由**：不泄露数据、不执行代码；影响是**可用性**。
但它可被**任意网页**触发，且现有看门狗盯的是"停滞的 job"，**盯不到卡在解析上的请求**
⇒ 不会自愈。

**修法（三件，缺一不完整）**：
- ① 把 loopback 守卫**前移到 ASGI 中间件**（中间件在路由与依赖解析之前运行，
  可在**不读 body** 的前提下按 `Host`/`Origin` 直接 403/关闭连接）；
- ② 加**请求体大小硬上限**（现有 `UPLOAD_LIMITS.max_bytes` 是**端点内**的检查，
  同样晚于解析）；
- ③ 升级 `python-multipart`（§17.3.2）—— 0.0.21 的 7 条 CVE 里有 3 条就是
  "畸形头部/前导数据 ⇒ DoS"，与上面的实测形态**同源**。

### 17.3.2 S2（P1，**本轮新发现**）：依赖漏洞，且都在不可信输入路径上

`pip-audit -r requirements.txt` ⇒ **50 条公告**（去重后 26 条 CVE）。全部来自 4 个包：

| 包 | 锁定版本 | CVE 数 | 关键条目 | 建议 |
|---|---|---|---|---|
| **python-multipart** | 0.0.21 | **7** | `CVE-2026-24486` **任意文件写**（"Non-Default Configuration"）；`CVE-2026-40347` 大前导/尾随数据 DoS；`CVE-2026-42561` 无界 part 头 DoS；`CVE-2026-53540` 负 `Content-Length` 全量缓冲；`CVE-2026-53537/53538` 参数走私；`CVE-2026-53539` 二次时间解析 CPU DoS | **≥0.0.31** |
| **Pillow** | 10.1.0 | **17** | `CVE-2023-50447` **任意代码执行**（`ImageMath.eval`）；`CVE-2024-28219` 缓冲区溢出；`CVE-2026-59199` 堆越界写（`Image.paste`/`crop`）；`CVE-2026-59205/59197` 堆越界写；多条解压炸弹绕过 / JPEG2000 / PDF 解析 DoS | **12.3.0**（差 4 个大版本）|
| requests | 2.32.4 | 1 | `CVE-2026-25645` `extract_zipped_paths()` 临时文件复用 | 2.33.0 |
| python-dotenv | 1.2.1 | 1 | `CVE-2026-28684` `set_key` 符号链接跟随 → 任意文件覆盖 | 1.2.2 |

**为什么这两条是阻断级**：

- `python-multipart` **解析每一次上传**（FastAPI `UploadFile` 的必需依赖）——
  不可信字节必经之地；
- `Pillow` 在 **`api/jobs/upload.py:216-269`** 对**用户上传的图片**调用
  `Image.open` → `ImageOps.exif_transpose` → `convert("RGBA")` → **`Image.paste`**
  → `save(PNG)`。其中 `paste` 正是 `CVE-2026-59199` 点名的函数。
  另有 `core/pipeline/self_heal.py:176` 对 OCR 页图调用 `Image.resize`。

⚠️ **必须说清的边界（不得当作"已确认可利用"）**：
- 这**不是**"已证明可被远程利用"，而是"**依赖落后 + 该依赖处于不可信输入路径**"
  两条**独立成立**的事实叠加。逐条 CVE 的可达性需**单独分析**（见 §17.8 留白）；
- `Pillow` 的两条最危险项中，`ImageMath` **本应用未使用**；`paste` 虽被调用，
  但坐标来自 `rgba.size`，攻击者不直接控制 `box` 参数 —— 可利用性**待验**；
- `requests` 的 `extract_zipped_paths` 与 `python-dotenv` 的 `set_key`
  **全仓零调用**（已 grep 核实：`config.py` 只用 `dotenv_values` / `load_dotenv`）
  ⇒ 这 2 条**当前不可达**，属"升级顺手带走"，**不构成阻断**。

**门禁为什么没抓到**：`release_gate.py` 的 9 项里**没有任何一项查依赖漏洞或
框架生命周期**。这是本轮暴露的**门禁盲区**，不是"忘了跑"。

### 17.3.3 S3（P2，**本轮新发现**）：运行时已过安全支持期

从**产物二进制里**读出的身份（不采信 `package.json` 的 `^33.0.0` 范围）：

```
Electron/33.4.11   Chrome/130.0.6723.191   node.js/v20.18.3
```

| 组件 | 本产物 | 现状（2026-09） | 差距 |
|---|---|---|---|
| Electron | 33.4.11 | 支持线 **41 / 42 / 43**（最新 43.x） | **33 于 2025-04-29 EOL** ⇒ 停安全支持 **17 个月** |
| Chromium | 130 | 稳定线 **M156**（对应 Electron 45）| 落后 **26 个大版本** |
| Node.js | 20.18.3 | Node 20 已 EOL | 落后 |

**为什么对制药客户更要紧**：GMP 环境通常有"不得运行已停止安全支持的组件"这类
供应商审计条款；本地应用读 PDF、渲染 HTML，Chromium 是**暴露面最大的组件**。

**修法**：升到受支持线并重建产物。⚠️ Electron 跨 8 个主版本升级**不会是一次
无害的改动** —— `contextIsolation`/`sandbox` 默认值、`setWindowOpenHandler`
行为、`webContents` 事件、以及打包器（electron-builder 25 → 当前）都可能变
⇒ 必须走"升版 → 重建 → **win-unpacked 级 e2e（B9-10 的驱动正好用得上）**"的完整链路。

### 17.3.4 已具备的安全加固（复核为**正确**，不推翻）

本轮逐条复核，以下**做对了**，不要因为上面三条就把结论说成"安全很差"：

| 项 | 实现 | 复核 |
|---|---|---|
| 进程隔离 | `contextIsolation: true` + `nodeIntegration: false`（splash 与主窗口**都**设了） | ✅ 正确。这是把"LLM/OCR 内容 → XSS → RCE"这条链**在根上掐断**的关键一项 |
| 外链 | `setWindowOpenHandler` **仅**放行 `http(s)` 交给系统浏览器，其余（`file:` / `javascript:` / `data:`）一律 deny | ✅ 正确（注释记录这是 P2-N 修过的） |
| 同窗导航 | `will-navigate` 白名单到 `127.0.0.1:PORT` / `localhost:PORT` | ✅ 正确，堵住钓鱼式导航 |
| SSRF | `core/security.py::_is_blocked_host` 覆盖 loopback/link-local/private/unspecified，**并处理非点分字面量**（`2130706433` / `0x7f000001` / `017700000001`）与 `localhost` 别名 | ✅ 超出常规水平 |
| 凭据脱敏 | `api/settings/read.py::_mask` 单点实现，`GET /api/settings` 只回掩码 | ✅ 未发现回传原文的端点 |
...
## 17.4 功能面：应用层可跑，但**最贵的两条路径仍未验过**

- **win-unpacked 级 e2e（B9-10 驱动）5 次连跑：35 passed / 0 failed / 5 skipped**，
  D1/D5/D6 时间高度一致（`e2e_unpacked_20260923_101909.json`）。
- **未覆盖（如实记 skipped，不冒充通过）**：
  - **LLM 成功路径**：凭据仍被上游拒绝（401 `code=30014`）⇒
    `review`/`partial_review` 终态与 findings 生成**本轮仍未真验过**（B9-7 的 `UNMET` 正因此年年挂着）；
  - **OCR 成功路径**：本轮无 `PADDLE_TOKEN`/`MINERU_TOKEN` ⇒ 未跑；
  - **D8 数据隔离**：如实 SKIP —— `%APPDATA%\PBC\logs\backend-boot.log` 每轮都会被写
    （Electron 的 `bootLog` 走 `app.getPath('appData')`，环境变量管不到）；
  - **200 页上限**（C2）仍是**线性外推未实跑**；51 页已实跑；
  - **只有 1 份真实批记录**（A3）⇒ 规则/OCR 的**泛化性未经检验**。
- **未闭环的关闭语义**：B9-2（整机关闭无自动化 e2e）、B9-3（关闭事件无审计落库）、
  B9-4（dev 模式 e2e 隔离不成立）、B9-8（残壳仍被宿主锁）。

## 17.5 精确性与准确性：**当前没有可外推的数字**（这是诚实的答案）

**唯一的 P/R/F1 = 1.0 / 1.0 / 1.0（TP=10, FP=0, FN=0）来自合成集**
（`docs/FINDING_GROUND_TRUTH.json`，11 个 case，输入由 `scripts/synthetic_corpus.py`
构造）⇒ **结果不可外推**，任何"准确率 100%"的表述都是拿合成集当成绩单。

真实数据上能拿到的**只有代理指标**：

| 指标 | 数值 | 出处与边界 |
|---|---|---|
| 51 页真实回放，降噪前 | **442** 条 findings（job `0c5cfb00-897`）| 单份记录，`§9.2` |
| 叠加 R1+M2 后 | **272** 条（**−38.5%**），噪声占比 75.8% → 60.7% | **离线重放/模型化估计**，非重跑流水线 |
| critical 条数 | **29 → 29**（降噪不碰 critical）| 同上 |
| Round 42 真实回放 | **293** 条 / **14** 类 | `§15.5` |
| Round 43 抽样直读原页 | 核 5 页 → **坐实 4 条假 critical**（+1 条确认）| **抽样，不可外推为整体比例** |
| Round 46 修复后（51 页回放） | 抑制 26→**13**、降级 42→**55**、保留 **88**、critical 假阳性 **0→0** | `§16.5` |
| 知识库检索 | 36/37 = **97.3%**（金标 37 条，threshold 0.85）| **属 KB 检索，不是 finding 精度** |

**已记录的误检/漏检实例**（说明"精度不是 1.0"）：

- `_decimal_loss_factor` 软化过宽，曾**吃掉真实超差** `25 vs ≤5.0 ℃`（`NOISE_REDUCTION_TODO.md:357`）；
- `llm_finding_guard._check_declared_order` 修复前把"方向反"一律**抑制**，
  吞掉 p27（年份差 **10 年**）与 p38 的 2027 未来日期（`§16.2` / `§16.5`）；
- **R2 批号投票在新抽取产物上"一次未触发"（0 voted）** —— 判据存在但**无判别力**；
- `stage2` 对损坏 JSON **fail-open**，把该页计入"已分析" ⇒ **静默漏检**（`§16.3` 附近）。

**结论**：`C1 无真实标注集 ⇒ 无可信 P/R` 这条**至今成立且是本轮最重要的精度结论**。
要把"精确性/准确性"从**叙事**变成**度量**，必须先做 **B1-3（建立真实标注集）**——
在此之前，所有精度讨论都只能在"抽样 + 代理指标"这个层级上谈，**不得上报表级数字**。

## 17.6 噪音控制：机制已经成体系，缺的是**闭环度量**

**四层 LLM 收权**（`core/rules/llm_finding_guard.py`，原则 = **弱证据只降级，强证据才抑制**）：

| 层 | 判据 | 动作 |
|---|---|---|
| L1 值形态 | 非数值形态 / 布尔错配 / 查无此值 | **抑制** |
| L2 溯源 | 文案长数字不在 OCR 原文中 | 降级 |
| L3 判据重算 | time_reversal **三态**：有倒序⇒保留｜均有可解析工序且无倒序⇒抑制｜**一个都解析不出⇒降级**；方向反且两侧皆记录内日期⇒降级；参照物为"当前日期"⇒抑制 | 抑制/降级 |
| L4 severity 封顶 | LLM 独断 critical 但无 L3 背书 | **无条件**降 warning |

**规则层/落库期**：R1 自指元噪声文档级聚合（<2 条不聚合）｜M2 completeness
（抽取依赖 `time` 且页有 OCR 告警 ⇒ info + `extraction_uncertain`；同类 ≥8 条且覆盖 ≥30% 页 ⇒ 折叠 1 条）
｜R3 年份投票**五闸**（单字符差 + 多数 ≥N 页 + 投票决定性 + 不相邻 + 批号佐证保护）
｜B1-1 同根因**发射期**聚合（组内 ≥2 才并、键含根因）｜`spec_guard` 用规则层同一解析器复核 LLM 自报超差，
命中 in/soft 才剔除、**否则 fail-closed 保留**。

**严重度三档**：`critical > warning > info`。降级路径：critical→warning（L4）、
warning→info（弱证据）、warning→info+`extraction_uncertain`（M2）；
另有**重分类交人工**（`spec_unverifiable` / `needs_arbitration` / `structure_warning`）。

**量化口径**：`noise_ratio = completeness 条数 / total`；降噪前后比
`reduction_ratio`；工具要求**页清单并集必须相等**（证据不丢）；
**硬约束：P/R/F1 不得下降**（行业禁则原文即"假阳率减半却丢真命中 = 控制失效"）。

**缺口（这就是"如何控制噪音"的下一步）**：

1. **`finding_suppressions` 台账在真实库 0 行** ⇒ 那个"−65.8%"只是离线重放，
   **不是流水线实测**。降噪效果**无法在生产上被审计**。
2. `eval_noise_reduction.py` **明示非端到端**。
3. `needs_arbitration` / `structure_warning` / `suppressed` 的 **KPI 未补**
   ⇒ "降噪有没有过头"目前**没有自动判据**，只能靠人读原页（§16.2、§17.5 的假阳性就是这么发现的）。
4. **R2 判据无判别力**（0 voted）—— 有机制、无效果。

⇒ **控制噪音的正解不是"再多抑制几条"**，而是三件事：
① **把降噪量变成可在生产上审计的数字**（suppressions 落库 + KPI）；
② **守住"漏检代价 > 误报代价"**（GMP 场景的取向已写进项目原则）；
③ **优先修"假阳性"而不是"多抑制"** —— B1-15（p39/p45/p46 勾选为否全是假阳性）
与 B1-17（同页同项**重复发射**）是**根因级**的噪声源，比继续加抑制规则更有效。

## 17.7 下一版（v1.2.1）迭代建议：**安全收口优先**

> 排序原则：**先堵"能被外部触发"的，再做"能被度量"的，最后做"能被优化"的**。
> 「改了进入产物的东西 ⇒ 必须升版 + 重建」这条纪律不变。

### 第一优先 · 安全收口（建议 v1.2.1 的核心范围）

| # | 事项 | 验收 |
|---|---|---|
| **V1** | 升级 4 个依赖：`python-multipart ≥0.0.31`、`Pillow →12.3.0`、`requests →2.33.0`、`python-dotenv →1.2.2` | `pip-audit` 归零；全量 pytest + 门禁 9/9；**Pillow 跨 4 个大版本，必须复跑图片上传路径**（`_image_to_pdf_sync` 的 RGBA 白底合成、多页 TIFF 拒绝、1 亿像素上限） |
| **V2** | **守卫前移到 ASGI 中间件**（体解析之前）+ 请求体大小硬上限 | 复用 `probe_browser_scenario.py`：畸形大体 + 跨站 `Origin` ⇒ 必须在**毫秒级**返回 403/413，且耗时**不随体积增长**（这是可判定的验收判据，不是"感觉快了"）|
| **V3** | Electron 33 → 受支持线并重建 | 产物内 `Chrome/` 版本刷新；**win-unpacked 级 e2e（B9-10 驱动）全绿**；`contextIsolation`/`nodeIntegration` 复核不变；`will-navigate`/`setWindowOpenHandler` 行为复测 |
| **V4** | **把本轮的两个盲区做成门禁第 10/11 项**：依赖漏洞扫描 + 运行时 EOL 检查 | 门禁能对"依赖出现已知 CVE"与"Electron/Chromium 超出支持线"给出**具名 FAIL**（不是 WARN）；变异验证：注入一个已知漏洞版本 ⇒ 必须红 |

⚠️ **V1/V3 会改产物 ⇒ 必须升版 + 重建**，而重建受**宿主持锁**（B9-8）阻塞
⇒ **需要用户先完全退出宿主**（这是 §16.4 就已确认的唯一解锁方式）。

### 第二优先 · 让精度与噪音变成**可度量**

| # | 事项 | 说明 |
|---|---|---|
| **V5** | **B1-3 建立真实标注集** | 这是所有精度讨论的**前提**。建议：从已回放过的 51 页里选 5–8 页做**人工逐条裁决**（真/假/存疑），先立一个**小而真**的标注集，再谈 P/R |
| **V6** | **B1-15（勾选为否假阳性）+ B1-17（同页同项重复发射）** | 根因级噪声源，收益高于继续加抑制规则 |
| **V7** | 降噪闭环度量：`finding_suppressions` **落库** + `needs_arbitration`/`structure_warning` KPI | 让"降噪有没有过头"有**自动判据** |

### 第三优先 · 工程与性能

`B2-3`/`B2-4`（CPU 移出事件循环）｜`B7-2`（ruff 105 条进门禁）｜
`B8`（发版 7 条不变量）｜`B9-2`/`B9-3`/`B9-4`（关闭语义收尾）｜
`C2`（≥120 页实跑）｜`A3`（第 2 份真实批记录）。

## 17.8 未复核 / 诚实留白（不得当成"已通过"）

1. **S2 的逐条 CVE 可达性未验证。** 只验证了"版本落后"与"两个包处于不可信输入路径"。
   `CVE-2026-24486`（任意文件写）明确写着 "Non-Default Configuration"，
   FastAPI 的默认用法**可能不受影响** —— 本节**没有**下"可利用"的结论。
2. **未做动态验证**：没有对任一 CVE 构造 PoC。Pillow 的 `Image.paste` 虽被调用，
   但 `box` 参数不由攻击者控制 ⇒ 可利用性**未知**。
3. **Electron 的 Chromium CVE 清单未逐条拉取**（只核了 EOL 日期与支持线）。
4. **内存测量是单次采样**（699 MB），未测长时运行是否有增长（无泄漏结论**未下**）。
5. **`npm audit`（Node 侧）未跑** —— 本轮只做了 Python 侧。`docx` / `tailwindcss` /
   `electron-builder` 的传递依赖**未审**。
6. **CSRF 的"浏览器可发送"是基于 CORS 规范与项目自身注释的推理 + 服务端实测**，
   **没有**用真实浏览器从 `http://evil` 页面发起验证（服务端行为已实测，浏览器侧未实测）。
7. **D8 隔离盲区仍在**（`bootLog` 写真实 `%APPDATA%`）。

## 17.9 复现命令汇总

```bash
PY="C:/Users/WuSiTan/AppData/Local/Programs/Python/Python311/python.exe"
"$PY" -m pip_audit -r requirements.txt --progress-spinner off     # 50 条公告
"$PY" devlogs/_verify/osv_detail.py                                # CVE 号/修复版本
"$PY" devlogs/_verify/probe_browser_scenario.py                    # S1 的核心证据
"$PY" devlogs/_verify/probe_upload_throughput.py                   # 正常 vs 畸形吞吐
"$PY" devlogs/_verify/probe_parse_before_guard.py                  # 守卫时序（Host 维度）
"$PY" devlogs/_verify/probe_perf_audit.py                          # 延迟/内存/启动
"$PY" scripts/release_gate.py                                      # 9 项门禁（现状全绿）
```

⚠️ 以上探针均**只读仓库**、**隔离 `APPDATA`**、只对**本机回环**的临时实例发请求，
结束前主动 `POST /api/shutdown` 并兜底清理 `pbc-server.exe` 残留。

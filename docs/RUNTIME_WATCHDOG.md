...
### 8.4 【中】e2e 轮次预算写死单值 → **三次**把真实长跑判成失败

同一类缺陷**已经犯过三次**，每次都是"写死单值"：

| 时间 | 现象 | 真相 |
|---|---|---|
| 2026-09-04 | rot 轮 620s > 默认 600s，被误杀 20s | 旋转证据本身完整 |
| 2026-09-16 | pdf 轮 835s > 默认 600s，判超时 | 该 job 之后正常进 `review`，835s 是真实耗时 |
| 2026-09-17 | **冻结冒烟**写死 60s 等终态 → 判 FAIL | 上游 Paddle 排队，单页 120s 仍在 `ocr_running`（服务端日志 `轮询 state=running pages=0/1`）；该轮其余 **17 项全 PASS**，只有这一项红 |

第三例的落点值得单记一笔：它出现在**产物级**冒烟里（`tests/e2e_frozen.py`），
正是"最后一道门"，且**同轮其它 17 项都绿** —— 若只看总失败数就下"产物有问题"
的结论会完全跑偏。定案依据是**隔离 appdata 的服务端日志**（作业仍在正常轮询），
而不是接口返回值。
...
**冻结冒烟（第三例）的修法**：终态等待改为**派生** `poll_timeout_for_pages(1) + 300`
（一次 1 页轮询封顶 + 分析基线），`E2E_FROZEN_TERMINAL_TIMEOUT` 可覆盖。
护栏加在该文件的**静态**检查里（它在 import 期就起服务，**不能**被 import）：
必须引用 `poll_timeout_for_pages` 与 env 覆盖，且不得再出现
`for … in range(N): time.sleep(...)` 的固定次数等待；另加一条产品侧护栏
"单页轮询封顶 > 实测单页耗时"（防止有人把 `POLL_TIMEOUT` 调到真实排队时间之下）。

...
一类**未修**问题：**OCR 把相邻单元格粘进同一个 `<td>`**（如 p9 批次号尾部与使用次数
粘成 `A000626-221201/417 次` → 使用次数手写实值 `17` 被读成 `417`；p6 同类使 `中和后 pH`
读成 `70 L 6.88`）。此类"数字本身即错"的条目**规则层与 LLM 层会被同时误报**，
任何"规格 vs 实测"的交叉复核都无法发现（规格正确、数字也看似合理），
故不加启发式软化——过宽的软化会抑制真实超差。方向留作后续专项：
抽取后校验（要求数字 token 与相邻文本间有分隔符）+ 手写值置信度标注。
另：仅含"时:分"的时点会被锚定到**当天**日期，可能产出 `signature_time_anomaly` 噪声。

...
- **硬上限 200 页**（`MAX_UPLOAD_PAGES`）：拒绝消息含**真实页数 + 上限 +
  ...
  ⚠️ 云厂商的 1,000~3,000 页**不可移植**：那是建立在其 1–2 s/页成本上的数字，
  可移植的量是**墙钟时间**而非页数。详见 [docs/UPLOAD_LIMITS.md](docs/UPLOAD_LIMITS.md)。
...
- **新增护栏**：`tests/unit/test_upload_limits.py`（21 项：纯函数边界语义含
  "等于上限放行/读取失败放行"、误配置收敛、单一真值派生、**防重新写死的源码扫描
  含正反对照**、策略 ≤ 厂商上限）+ `tests/integration/test_api_upload_page_limit.py`
